...
mindplot.layout = {};
Class.Mutators.Static = function(a) {
	this.extend(a)
};
mindplot.Events = new Class({
	$events: {},
	_removeOn: function(a) {
...
Options = new Class({
	setOptions: function() {
		...
		if (this.addEvent) {
			for (var b in a) {
				if (typeOf(a[b]) != "function" || !(/^on[A-Z]/).test(b)) {
					continue
				}
				this.addEvent(b, a[b]);
				delete a[b]
...
mindplot.TopicEventDispatcher = new Class({
	...
	Static: {
		...
		configure: function(a) {
			this._instance = new mindplot.TopicEventDispatcher(a)
		},
	...
	close: function(a) {
		if (this.isVisible()) {
			this._activeEditor.close(a);
			this._activeEditor = null
		}
	},
	show: function(b, a) {
		this.process(mindplot.TopicEvent.EDIT, b, a)
	},
	process: function(d, c, b) {
		$assert(d, "eventType can not be null");
		if (this.isVisible()) {
			this.close()
		}
		var a = c.getModel();
		if (a.getShapeType() != mindplot.model.TopicShape.IMAGE && !this._readOnly && d == mindplot.TopicEvent.EDIT) {
			this._multilineEditor.show(c, b ? b.text : null);
...
mindplot.model.IMindmap = new Class({
	...
	connect: function(a, b) {
		$assert(!b.getParent(), "Child model seems to be already connected");
		a.append(b);
		this.removeBranch(b)
	},
	...
	hasAlreadyAdded: function(a) {
		throw "Unsupported operation"
	},
	createNode: function(a, b) {
		throw "Unsupported operation"
	},
	createRelationship: function(b, a) {
		throw "Unsupported operation"
	},
	addRelationship: function(a) {
		throw "Unsupported operation"
	},
	deleteRelationship: function(a) {
		throw "Unsupported operation"
	},
	inspect: function() {
		var a = "";
	...
	copyTo: function(d) {
		...
		var b = c.getBranches();
		_.each(b, function(f) {
			var g = d.createNode(f.getType(), f.getId());
			f.copyTo(g);
			d.addBranch(g)
		})
	}
...
mindplot.model.Mindmap = new Class({
	...
	addBranch: function(b) {
		$assert(b && b.isNodeModel(), "Add node must be invoked with model objects");
		var a = this.getBranches();
		if (a.length == 0) {
			$assert(b.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE, "First element must be the central topic");
			b.setPosition(0, 0)
		} else {
			$assert(b.getType() != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE, "Mindmaps only have one cental topic")
		}
		this._branches.push(b)
	},
	removeBranch: function(a) {
		$assert(a && a.isNodeModel(), "Remove node must be invoked with model objects");
	...
	hasAlreadyAdded: function(d) {
		var a = false;
		var b = this._branches;
		for (var c = 0; c < b.length; c++) {
			a = b[c]._isChildNode(d);
			if (a) {
				break
			}
		}
	...
	createNode: function(a, b) {
		a = !$defined(a) ? mindplot.model.INodeModel.MAIN_TOPIC_TYPE : a;
		return new mindplot.model.NodeModel(a, this, b)
	},
	createRelationship: function(a, b) {
		$assert($defined(a), "from node cannot be null");
	...
	findNodeById: function(d) {
		var a = null;
		for (var b = 0; b < this._branches.length; b++) {
			var c = this._branches[b];
			a = c.findNodeById(d);
			if (a) {
				break
			}
...
mindplot.model.Mindmap.buildEmpty = function(b) {
	var a = new mindplot.model.Mindmap(b);
...
mindplot.model.INodeModel = new Class({
	initialize: function(a) {
		$assert(a && a.getBranches, "mindmap can not be null");
		this._mindmap = a
	},
	getId: function() {
	...
	setId: function(a) {
		if ($defined(a) && a > mindplot.model.INodeModel._uuid) {
			mindplot.model.INodeModel._uuid = a
		}
		if (!$defined(a)) {
			a = mindplot.model.INodeModel._nextUUID()
		}
	...
	setType: function(a) {
		this.putProperty("type", a)
	},
	setText: function(a) {
		this.putProperty("text", a)
	},
	getText: function() {
		return this.getProperty("text")
	},
	setPosition: function(a, b) {
		$assert(!isNaN(parseInt(a)), "x position is not valid:" + a);
	...
	setImageSize: function(b, a) {
		this.putProperty("imageSize", "{width:" + b + ",height:" + a + "}")
	},
	...
	setImageUrl: function(a) {
		this.putProperty("imageUrl", a)
	},
	getMetadata: function() {
		return this.getProperty("metadata")
	},
	setMetadata: function(a) {
		this.putProperty("metadata", a)
	},
	...
	setShapeType: function(a) {
		this.putProperty("shapeType", a)
	},
	setOrder: function(a) {
		$assert(typeof a === "number" && isFinite(a) || a == null, "Order must be null or a number");
	...
	setBorderColor: function(a) {
		this.putProperty("borderColor", a)
	},
	getBackgroundColor: function() {
		return this.getProperty("backgroundColor")
	},
	setBackgroundColor: function(a) {
		this.putProperty("backgroundColor", a)
	},
	areChildrenShrunken: function() {
		var a = this.getProperty("shrunken");
		return $defined(a) ? a : false
	},
	setChildrenShrunken: function(a) {
		this.putProperty("shrunken", a)
	},
	...
	connectTo: function(b) {
		$assert(b, "parent can not be null");
		var a = this.getMindmap();
		a.connect(b, this)
	},
	copyTo: function(e) {
		...
		_.each(c, function(f) {
			var g = d.getProperty(f);
			e.putProperty(f, g)
		});
		var b = this.getChildren();
		var a = e.getMindmap();
		_.each(function(g, f) {
			var h = a.createNode(f.getType(), f.getId());
			f.copyTo(h);
			e.append(h)
		});
		return e
	...
	inspect: function() {
		var a = "{ type: " + this.getType() + " , id: " + this.getId() + " , text: " + this.getText();
		var b = this.getChildren();
		if (b.length > 0) {
			a = a + ", children: {(size:" + b.length;
			_.each(b, function(d) {
				a = a + "=> (";
				var c = d.getPropertiesKeys();
				_.each(c, function(e) {
					var f = d.getProperty(e);
					a = a + e + ":" + f + ","
				});
				a = a + "}"
			})
...
mindplot.model.NodeModel = new Class({
	...
	initialize: function(b, a, c) {
		$assert(b, "Node type can not be null");
	...
	createFeature: function(b, a) {
		return mindplot.TopicFeature.createModel(b, a)
	},
	addFeature: function(a) {
		$assert(a, "feature can not be null");
	...
	removeFeature: function(b) {
		...
		this._feature = this._feature.filter(function(c) {
			return b.getId() != c.getId()
		});
		$assert(a - 1 == this._feature.length, "Could not be removed ...")
	},
	findFeatureByType: function(a) {
		$assert(a, "type can not be null");
		return this._feature.filter(function(b) {
			return b.getType() == a
		})
	},
	findFeatureById: function(b) {
		$assert($defined(b), "id can not be null");
		var a = this._feature.filter(function(c) {
			return c.getId() == b
		});
	...
	getProperty: function(b) {
		$defined(b, "key can not be null");
		var a = this._properties[b];
		return !$defined(a) ? null : a
	},
	clone: function() {
		var a = new mindplot.model.NodeModel(this.getType(), this._mindmap);
		a._children = this._children.map(function(c) {
			var b = c.clone();
			b._parent = a;
			return b
		});
	...
	deepCopy: function() {
		var a = new mindplot.model.NodeModel(this.getType(), this._mindmap);
		a._children = this._children.map(function(d) {
			var c = d.deepCopy();
			c._parent = a;
			return c
		});
	...
	append: function(a) {
		$assert(a && a.isNodeModel(), "Only NodeModel can be appended to Mindmap object");
	...
	setParent: function(a) {
		$assert(a != this, "The same node can not be parent and child if itself.");
		this._parent = a
	},
	_isChildNode: function(d) {
		var a = false;
		if (d == this) {
			a = true
		} else {
...
mindplot.model.RelationshipModel = new Class({
	...
	initialize: function(b, a) {
		$assert($defined(b), "from node type can not be null");
	...
	setLineType: function(a) {
		this._lineType = a
	},
	getSrcCtrlPoint: function() {
		return this._srcCtrlPoint
	},
	setSrcCtrlPoint: function(a) {
		this._srcCtrlPoint = a
	},
	getDestCtrlPoint: function() {
		return this._destCtrlPoint
	},
	setDestCtrlPoint: function(a) {
		this._destCtrlPoint = a
	},
	...
	clone: function() {
		var a = new mindplot.model.RelationshipModel(this._sourceTargetId, this._targetTopicId);
		a._id = this._id;
...
mindplot.ActionDispatcher = new Class({
	...
	initialize: function(a) {
		$assert(a, "commandContext can not be null")
	},
	addRelationship: function(b, a) {
		throw "method must be implemented."
	},
	addTopics: function(b, a) {
		throw "method must be implemented."
	},
	deleteEntities: function(a, b) {
		throw "method must be implemented."
	},
	dragTopic: function(d, b, a, c) {
		throw "method must be implemented."
	},
	moveTopic: function(b, a) {
		throw "method must be implemented."
	},
	moveControlPoint: function(b, a) {
		throw "method must be implemented."
	},
	changeFontFamilyToTopic: function(b, a) {
		throw "method must be implemented."
	},
	changeFontStyleToTopic: function(a) {
		throw "method must be implemented."
	},
	changeFontColorToTopic: function(b, a) {
		throw "method must be implemented."
	},
	changeFontSizeToTopic: function(a, b) {
		throw "method must be implemented."
	},
	changeBackgroundColorToTopic: function(b, a) {
		throw "method must be implemented."
	},
	changeBorderColorToTopic: function(b, a) {
		throw "method must be implemented."
	},
	changeShapeTypeToTopic: function(a, b) {
		throw "method must be implemented."
	},
	changeFontWeightToTopic: function(a) {
		throw "method must be implemented."
	},
	changeTextToTopic: function(a, b) {
		throw "method must be implemented."
	},
	shrinkBranch: function(a, b) {
		throw "method must be implemented."
	},
	addFeatureToTopic: function(c, b, a) {
		throw "method must be implemented."
	},
	changeFeatureToTopic: function(b, c, a) {
		throw "method must be implemented."
	},
	removeFeatureFromTopic: function(a, b) {
		throw "method must be implemented."
	}
});
mindplot.ActionDispatcher.setInstance = function(a) {
	mindplot.ActionDispatcher._instance = a
};
...
mindplot.StandaloneActionDispatcher = new Class({
	...
	addTopics: function(c, b) {
		var a = new mindplot.commands.AddTopicCommand(c, b);
		this.execute(a)
	},
	addRelationship: function(a) {
		var b = new mindplot.commands.AddRelationshipCommand(a);
		this.execute(b)
	},
	deleteEntities: function(a, b) {
		var c = new mindplot.commands.DeleteCommand(a, b);
		this.execute(c)
	},
	dragTopic: function(d, b, a, c) {
		var e = new mindplot.commands.DragTopicCommand(d, b, a, c);
		this.execute(e)
	},
	moveTopic: function(b, a) {
		$assert($defined(b), "topicsId can not be null");
	...
	moveControlPoint: function(b, a) {
		var c = new mindplot.commands.MoveControlPointCommand(b, a);
		this.execute(c)
	},
	changeFontStyleToTopic: function(a) {
		var b = function(e) {
			var d = e.getFontStyle();
	...
	changeTextToTopic: function(a, d) {
		$assert($defined(a), "topicsIds can not be null");
		var b = function(f, g) {
			var e = f.getText();
			f.setText(g);
			return e
	...
	changeFontFamilyToTopic: function(d, a) {
		$assert(d, "topicIds can not be null");
		$assert(a, "fontFamily can not be null");
		var b = function(g, f) {
			var e = g.getFontFamily();
	...
	changeFontColorToTopic: function(b, a) {
		$assert(b, "topicIds can not be null");
		$assert(a, "color can not be null");
		var c = function(g, f) {
			var e = g.getFontColor();
	...
	changeShapeTypeToTopic: function(a, b) {
		$assert(a, "topicsIds can not be null");
		$assert(b, "shapeType can not be null");
		var c = function(f, g) {
			var e = f.getShapeType();
	...
	shrinkBranch: function(a, d) {
		$assert(a, "topicsIds can not be null");
		var b = function(f, e) {
			f.setChildrenShrunken(e);
			return !e
		};
		var c = new mindplot.commands.GenericFunctionCommand(b, a, d);
	...
	addFeatureToTopic: function(c, b, a) {
		var d = new mindplot.commands.AddFeatureToTopicCommand(c, b, a);
		this.execute(d)
	},
	changeFeatureToTopic: function(b, d, a) {
		var c = new mindplot.commands.ChangeFeatureToTopicCommand(b, d, a);
		this.execute(c)
	},
	removeFeatureFromTopic: function(a, c) {
		var b = new mindplot.commands.RemoveFeatureFromTopicCommand(a, c);
		this.execute(b)
	},
	execute: function(a) {
		this._actionRunner.execute(a)
	}
});
mindplot.CommandContext = new Class({
	initialize: function(a) {
		$assert(a, "designer can not be null");
		this._designer = a
	},
	findTopics: function(b) {
		$assert($defined(b), "topicsIds can not be null");
		if (!(b instanceof Array)) {
			b = [b]
		}
		var d = this._designer.getModel().getTopics();
		var a = d.filter(function(e) {
			return b.contains(e.getId())
		});
	...
	deleteTopic: function(a) {
		this._designer.removeTopic(a)
	},
	createTopic: function(a) {
		$assert(a, "model can not be null");
		return this._designer.nodeModelToNodeGraph(a)
	},
	createModel: function() {
		var a = this._designer.getMindmap();
	...
	connect: function(b, a) {
		b.connectTo(a, this._designer._workspace)
	},
	disconnect: function(a) {
		a.disconnect(this._designer._workspace)
	},
	addRelationship: function(a) {
		$assert(a, "model cannot be null");
		return this._designer.addRelationship(a)
	},
	deleteRelationship: function(a) {
		this._designer.deleteRelationship(a)
	},
	findRelationships: function(b) {
		$assert($defined(b), "relId can not be null");
		if (!(b instanceof Array)) {
			b = [b]
		}
		var a = this._designer.getModel().getRelationships();
		return a.filter(function(c) {
			return b.contains(c.getId())
		})
	},
	moveTopic: function(b, a) {
		$assert(b, "topic cannot be null");
...
mindplot.DesignerModel = new Class({
	...
	getCentralTopic: function() {
		var a = this.getTopics();
		return a[0]
	},
	filterSelectedTopics: function() {
		var a = [];
		for (var b = 0; b < this._topics.length; b++) {
			if (this._topics[b].isOnFocus()) {
				a.push(this._topics[b])
			}
		}
		return a
	},
	filterSelectedRelationships: function() {
		var a = [];
		for (var b = 0; b < this._relationships.length; b++) {
			if (this._relationships[b].isOnFocus()) {
				a.push(this._relationships[b])
			}
		}
		return a
	},
	getEntities: function() {
		var a = [].append(this._topics);
		a.append(this._relationships);
		return a
	},
	removeTopic: function(a) {
		$assert(a, "topic can not be null");
		this._topics.erase(a)
	},
	removeRelationship: function(a) {
		$assert(a, "rel can not be null");
		this._relationships.erase(a)
	},
	addTopic: function(a) {
		$assert(a, "topic can not be null");
	...
	selectedTopic: function() {
		var a = this.filterSelectedTopics();
		return (a.length > 0) ? a[0] : null
	},
	findTopicById: function(d) {
		var a = null;
		for (var c = 0; c < this._topics.length; c++) {
			var b = this._topics[c];
			if (b.getId() == d) {
				a = b;
				break
			}
...
mindplot.Designer = new Class({
	Extends: mindplot.Events,
	initialize: function(b, a) {
		$assert(b, "options must be defined");
		...
		var e = this;
		this._actionDispatcher.addEvent("modelUpdate", function(f) {
			e.fireEvent("modelUpdate", f)
		});
	...
	addEvent: function(b, c) {
		if (b == mindplot.TopicEvent.EDIT || b == mindplot.TopicEvent.CLICK) {
			var a = mindplot.TopicEventDispatcher.getInstance();
			a.addEvent(b, c)
		} else {
			this.parent(b, c)
		}
	...
	_registerMouseEvents: function() {
		var b = this._workspace;
		var d = b.getScreenManager();
		var c = this;
		d.addEvent("update", function() {
			var e = c.getModel().getTopics();
			_.each(e, function(f) {
				f.closeEditors()
			});
			if (c._cleanScreen) {
		...
		d.addEvent("click", function(e) {
			c.onObjectFocusEvent(null, e)
		});
		d.addEvent("dblclick", function(g) {
			if (b.isWorkspaceEventsEnabled()) {
				var e = d.getWorkspaceMousePosition(g);
	...
	_buildDragManager: function(c) {
		var b = this.getModel();
		var a = new mindplot.DragConnector(b, this._workspace);
		var d = new mindplot.DragManager(c, this._eventBussDispatcher);
		var e = b.getTopics();
		d.addEvent("startdragging", function() {
			for (var f = 0; f < e.length; f++) {
				e[f].setMouseEventsEnabled(false)
			}
		});
		d.addEvent("dragging", function(g, f) {
			f.updateFreeLayout(g);
			if (!f.isFreeLayoutOn(g)) {
				a.checkConnection(f);
				if (!f.isVisible() && f.isConnected()) {
					f.setVisibility(true)
				}
			}
		});
		d.addEvent("enddragging", function(h, g) {
			for (var f = 0; f < e.length; f++) {
				e[f].setMouseEventsEnabled(true)
			}
			g.applyChanges(c)
		});
		return d
	...
	setViewPort: function(b) {
		this._workspace.setViewPort(b);
		var a = this.getModel();
		this._workspace.setZoom(a.getZoom(), true)
	},
	_buildNodeGraph: function(e, j) {
		var f = mindplot.NodeGraph.create(e, {
			readOnly: j
		});
		this.getModel().addTopic(f);
		var h = this;
		if (!j) {
			f.addEvent("mousedown", function(i) {
				h.onObjectFocusEvent(f, i)
			});
			if (f.getType() != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
				this._dragManager.add(f)
			}
		}
		var d = e.isConnected();
		if (d) {
		...
		f.addEvent("ontblur", function() {
			var l = h.getModel().filterSelectedTopics();
			var i = h.getModel().filterSelectedRelationships();
			if (l.length == 0 || i.length == 0) {
				h.fireEvent("onblur")
			}
		});
		f.addEvent("ontfocus", function() {
			var l = h.getModel().filterSelectedTopics();
			var i = h.getModel().filterSelectedRelationships();
	...
	onObjectFocusEvent: function(a, c) {
		var e = this.getModel().getTopics();
		_.each(e, function(f) {
			f.closeEditors()
		});
		var b = this.getModel();
		var d = b.getEntities();
		_.each(d, function(f) {
			if (!$defined(c) || (!c.ctrlKey && !c.metaKey)) {
				if (f.isOnFocus() && f != a) {
					f.setOnFocus(false)
				}
			}
	...
	deselectAll: function() {
		var a = this.getModel().getEntities();
		_.each(a, function(b) {
			b.setOnFocus(false)
		})
	},
	...
	copyToClipboard: function() {
		...
		a = a.filter(function(b) {
			return !b.isCentralTopic()
		});
		this._clipboard = a.map(function(b) {
			var d = b.getModel().deepCopy();
			var c = d.getPosition();
	...
	shrinkSelectedBranch: function() {
		...
		var b = a[0];
		if (b.getType() != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
	...
	_copyNodeProps: function(d, a) {
		if (d.getType() != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			var j = d.getFontSize();
			if (j) {
				a.setFontSize(j)
			}
	...
	_createChildModel: function(g, c) {
		var f = g.getModel();
		var e = f.getMindmap();
		var h = e.createNode();
	...
	addDraggedNode: function(c, b) {
		$assert(c, "event can not be null");
	...
	loadMap: function(b) {
		$assert(b, "mindmapModel can not be null");
		this._mindmap = b;
		var m = {
			width: 25,
			height: 25
		...
		for (var e = 0; e < g.length; e++) {
			this._relationshipModelToRelationship(g[e])
		}
	...
	nodeModelToNodeGraph: function(e) {
		$assert(e, "Node model can not be null");
		var c = e.getChildren().slice();
		c = c.sort(function(g, f) {
			return g.getOrder() - f.getOrder()
		});
		...
		for (var b = 0; b < c.length; b++) {
			var d = c[b];
			if ($defined(d)) {
				this.nodeModelToNodeGraph(d)
			}
		}
		return a
	},
	_relationshipModelToRelationship: function(d) {
		$assert(d, "Node model can not be null");
	...
	_buildRelationshipShape: function(d) {
		var g = this.getModel();
		var f = d.getFromNode();
		var b = g.findTopicById(f);
		var e = d.getToNode();
		var c = g.findTopicById(e);
		$assert(c, "targetTopic could not be found:" + e + g.getTopics().map(function(i) {
			return i.getId()
		}));
		var a = new mindplot.Relationship(b, c, d);
		var h = this;
		a.addEvent("ontblur", function() {
			var j = h.getModel().filterSelectedTopics();
			var i = h.getModel().filterSelectedRelationships();
			if (j.length == 0 || i.length == 0) {
				h.fireEvent("onblur")
			}
		});
		a.addEvent("ontfocus", function() {
			var j = h.getModel().filterSelectedTopics();
			var i = h.getModel().filterSelectedRelationships();
	...
	deleteSelectedEntities: function() {
		...
		if (c.length > 0 || a.length > 0) {
			this._actionDispatcher.deleteEntities(c, a)
		}
	},
	changeFontFamily: function(b) {
		var a = this.getModel().filterTopicsIds();
		if (a.length > 0) {
			this._actionDispatcher.changeFontFamilyToTopic(a, b)
		}
	},
	changeFontStyle: function() {
		var a = this.getModel().filterTopicsIds();
		if (a.length > 0) {
			this._actionDispatcher.changeFontStyleToTopic(a)
		}
	},
	changeFontColor: function(b) {
		$assert(b, "color can not be null");
		var a = this.getModel().filterTopicsIds();
		if (a.length > 0) {
			this._actionDispatcher.changeFontColorToTopic(a, b)
		}
	},
	changeBackgroundColor: function(c) {
		var d = function(e) {
			return e.getShapeType() != mindplot.model.TopicShape.LINE
		};
		var a = "Color can not be set to line topics.";
		var b = this.getModel().filterTopicsIds(d, a);
		if (b.length > 0) {
			this._actionDispatcher.changeBackgroundColorToTopic(b, c)
		}
	},
	changeBorderColor: function(c) {
		var d = function(e) {
			return e.getShapeType() != mindplot.model.TopicShape.LINE
		};
		var a = "Color can not be set to line topics.";
		var b = this.getModel().filterTopicsIds(d, a);
		if (b.length > 0) {
			this._actionDispatcher.changeBorderColorToTopic(b, c)
		}
	},
	changeFontSize: function(b) {
		var a = this.getModel().filterTopicsIds();
		if (a.length > 0) {
			this._actionDispatcher.changeFontSizeToTopic(a, b)
		}
	},
	changeTopicShape: function(c) {
		var d = function(e) {
			return !(e.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE && c == mindplot.model.TopicShape.LINE)
		};
		var a = "Central Topic shape can not be changed to line figure.";
		var b = this.getModel().filterTopicsIds(d, a);
		if (b.length > 0) {
			this._actionDispatcher.changeShapeTypeToTopic(b, c)
		}
	},
	changeFontWeight: function() {
		var a = this.getModel().filterTopicsIds();
		if (a.length > 0) {
			this._actionDispatcher.changeFontWeightToTopic(a)
		}
	},
	addIconType: function(a) {
		var b = this.getModel().filterTopicsIds();
		if (b.length > 0) {
			this._actionDispatcher.addFeatureToTopic(b[0], mindplot.TopicFeature.Icon.id, {
				id: a
			})
		}
	},
...
mindplot.ScreenManager = new Class({
	initialize: function(a) {
		...
		this._clickEvents = [];
		this._divContainer.bind("click", function(b) {
			b.stopPropagation()
		});
		this._divContainer.bind("dblclick", function(b) {
			b.stopPropagation();
			b.preventDefault()
		})
	},
	setScale: function(a) {
		$assert(a, "Screen scale can not be null");
		this._scale = a
	},
	addEvent: function(a, b) {
		if (a == "click") {
			this._clickEvents.push(b)
		} else {
			this._divContainer.bind(a, b)
		}
	},
	removeEvent: function(a, b) {
		if (a == "click") {
			this._clickEvents.remove(b)
		} else {
			this._divContainer.unbind(a, b)
		}
	},
	fireEvent: function(a, b) {
		if (a == "click") {
			_.each(this._clickEvents, function(c) {
				c(a, b)
			})
		} else {
			this._divContainer.trigger(a, b)
		}
	},
	...
	getWorkspaceIconPosition: function(j) {
		var c = j.getImage();
		var d = c.getPosition();
		var k = j.getSize();
		var f = j.getGroup();
		...
		var b = {
			x: n.width / parseInt(p.width),
			y: n.height / parseInt(p.height)
		};
	...
	getWorkspaceMousePosition: function(b) {
		var a = b.clientX;
		var c = b.clientY;
		a = a * this._scale;
		c = c * this._scale;
...
mindplot.Workspace = new Class({
	initialize: function(d, c) {
		$assert(d, "Div container can not be null");
	...
	append: function(a) {
		if ($defined(a.addToWorkspace)) {
			a.addToWorkspace(this)
		} else {
			this._workspace.append(a)
		}
	},
	removeChild: function(a) {
		if ($defined(a.removeFromWorkspace)) {
			a.removeFromWorkspace(this)
		} else {
			this._workspace.removeChild(a)
		}
	},
	addEvent: function(a, b) {
		this._workspace.addEvent(a, b)
	},
	removeEvent: function(a, b) {
		$assert(a, "type can not be null");
	...
	setZoom: function(g, a) {
		...
		d.setCoordSize(c, h);
		if (this._viewPort) {
			this._viewPort.width = this._viewPort.width * g;
	...
	enableWorkspaceEvents: function(a) {
		this._eventsEnabled = a
	},
	...
	_registerDragEvents: function() {
		...
		var c = this;
		var a = function(g) {
			if (!$defined(b._mouseMoveListener)) {
				if (c.isWorkspaceEventsEnabled()) {
					c.enableWorkspaceEvents(false);
					var f = d.getWorkspaceMousePosition(g);
					var e = b.getCoordOrigin();
					var h = false;
					b._mouseMoveListener = function(l) {
						var n = d.getWorkspaceMousePosition(l);
						var i = n.x - f.x;
						var k = -i + e.x;
						var m = n.y - f.y;
						var j = -m + e.y;
					...
					b._mouseUpListener = function(j) {
						d.removeEvent("mousemove", b._mouseMoveListener);
...
mindplot.ShirinkConnector = new Class({
	initialize: function(b) {
		var d = new web2d.Elipse(mindplot.Topic.prototype.INNER_RECT_ATTRIBUTES);
		this._ellipse = d;
		...
		d.addEvent("mousedown", function(e) {
			e.stopPropagation()
		});
		d.addEvent("dblclick", function(e) {
			e.stopPropagation()
		});
		d.addEvent("mouseover", function(e) {
			d.setFill("rgb(153, 0, 255)")
		});
		var c = this;
		d.addEvent("mouseout", function(f) {
			var e = b.getBackgroundColor();
			c.setFill(e)
		});
		d.setCursor("default");
	...
	setVisibility: function(a) {
		this._ellipse.setVisibility(a)
	},
	setOpacity: function(a) {
		this._ellipse.setOpacity(a)
	},
	setFill: function(a) {
		this._fillColor = a;
	...
	setAttribute: function(a, b) {
		this._ellipse.setAttribute(a, b)
	},
	addToWorkspace: function(a) {
		a.append(this._ellipse)
	},
	setPosition: function(a, b) {
		this._ellipse.setPosition(a, b)
	},
	moveToBack: function() {
...
mindplot.DesignerKeyboard = new Class({
	...
	Static: {
		register: function(a) {
			this._instance = new mindplot.DesignerKeyboard(a)
		},
		getInstance: function() {
			return this._instance
		}
	...
	_registerEvents: function(b) {
		var a = b.getModel();
		this.addShortcut(["backspace"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.deleteSelectedEntities()
		});
		this.addShortcut(["space"], function() {
			b.shrinkSelectedBranch()
		});
		this.addShortcut(["f2"], function(f) {
			f.stopPropagation();
			f.preventDefault();
		...
		this.addShortcut(["del"], function(e) {
			b.deleteSelectedEntities();
			e.preventDefault();
			e.stopPropagation()
		});
		this.addShortcut(["enter"], function() {
			b.createSiblingForSelectedNode()
		});
		this.addShortcut(["insert"], function(e) {
			b.createChildForSelectedNode();
			e.preventDefault();
			e.stopPropagation()
		});
		this.addShortcut(["tab"], function(e) {
			b.createChildForSelectedNode();
			e.preventDefault();
			e.stopPropagation()
		});
		this.addShortcut(["meta+enter"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.createChildForSelectedNode()
		});
		this.addShortcut(["ctrl+z", "meta+z"], function(e) {
			e.preventDefault(e);
			e.stopPropagation();
			b.undo()
		});
		this.addShortcut(["ctrl+c", "meta+c"], function(e) {
			e.preventDefault(e);
			e.stopPropagation();
			b.copyToClipboard()
		});
		this.addShortcut(["ctrl+v", "meta+v"], function(e) {
			e.preventDefault(e);
			e.stopPropagation();
			b.pasteClipboard()
		});
		this.addShortcut(["ctrl+shift+z", "meta+shift+z", "ctrl+y", "meta+y"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.redo()
		});
		this.addShortcut(["ctrl+a", "meta+a"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.selectAll()
		});
		this.addShortcut(["ctrl+b", "meta+b"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.changeFontWeight()
		});
		this.addShortcut(["ctrl+s", "meta+s"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			$(document).find("#save").trigger("click")
		});
		this.addShortcut(["ctrl+i", "meta+i"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.changeFontStyle()
		});
		this.addShortcut(["ctrl+shift+a", "meta+shift+a"], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.deselectAll()
		});
		this.addShortcut(["meta+=", "ctrl+="], function(e) {
			e.preventDefault();
			e.stopPropagation();
			b.zoomIn()
		});
		...
		var c = this;
		this.addShortcut("right", function(f) {
			var e = a.selectedTopic();
			if (e) {
				if (e.isCentralTopic()) {
					c._goToSideChild(b, e, "RIGHT")
				} else {
					if (e.getPosition().x < 0) {
						c._goToParent(b, e)
					} else {
						if (!e.areChildrenShrunken()) {
							c._goToChild(b, e)
						}
					}
				}
			} else {
				var g = a.getCentralTopic();
				c._goToNode(b, g)
			}
			f.preventDefault();
			f.stopPropagation()
		});
		this.addShortcut("left", function(f) {
			var e = a.selectedTopic();
			if (e) {
				if (e.isCentralTopic()) {
					c._goToSideChild(b, e, "LEFT")
				} else {
					if (e.getPosition().x > 0) {
						c._goToParent(b, e)
					} else {
		...
		this.addShortcut("up", function(f) {
			var e = a.selectedTopic();
			if (e) {
				if (!e.isCentralTopic()) {
					c._goToBrother(b, e, "UP")
				}
			} else {
				var g = a.getCentralTopic();
				c._goToNode(b, g)
		...
		this.addShortcut("down", function(f) {
			var e = a.selectedTopic();
			if (e) {
				if (!e.isCentralTopic()) {
					c._goToBrother(b, e, "DOWN")
				}
			} else {
				var g = a.getCentralTopic();
				c._goToNode(b, g)
		...
		$(document).on("keypress", function(f) {
			var h;
			if (f.key && d.contains(f.key.toLowerCase())) {
				return
			}
			if (f.key && f.keyCode == 0) {
				h = f.charCode
			} else {
				h = f.keyCode
			}
			var g = jQuery.hotkeys.specialKeys[h];
			if (["enter", "capslock"].indexOf(g) == -1 && !jQuery.hotkeys.shiftNums[h]) {
				var e = b.getModel().filterSelectedTopics();
				if (e.length > 0) {
					var i = String.fromCharCode(h);
					if (f.ctrlKey || f.altKey || f.metaKey) {
						return
					}
	...
	_goToSideChild: function(j, c, f) {
		...
		if (b.length > 0) {
			...
			for (var d = 0; d < b.length; d++) {
				...
				if (f == "LEFT" && a.getPosition().x < 0) {
					if (g == null || h < g) {
						e = a;
						g = h
					}
				}
				if (f == "RIGHT" && a.getPosition().x > 0) {
					if (g == null || h < g) {
						e = a;
						g = h
					}
	...
	_goToParent: function(a, c) {
		var b = c.getParent();
		if (b) {
			this._goToNode(a, b)
		}
	},
	_goToChild: function(c, d) {
		var b = d.getChildren();
		if (b.length > 0) {
			var f = b[0];
...
mindplot.TopicStyle = new Class({
	Static: {
		_getStyles: function(c) {
			$assert(c, "topic can not be null");
			var a;
			if (c.isCentralTopic()) {
				a = mindplot.TopicStyle.STYLES.CENTRAL_TOPIC
			} else {
				var b = c.getOutgoingConnectedTopic();
				if ($defined(b)) {
					if (b.isCentralTopic()) {
						a = mindplot.TopicStyle.STYLES.MAIN_TOPIC
					} else {
						a = mindplot.TopicStyle.STYLES.SUB_TOPIC
					}
				} else {
					a = mindplot.TopicStyle.STYLES.ISOLATED_TOPIC
				}
			}
			return a
		},
		defaultText: function(a) {
			var b = this._getStyles(a).msgKey;
			return $msg(b)
		},
		defaultFontStyle: function(a) {
			return this._getStyles(a).fontStyle
		},
		defaultBackgroundColor: function(a) {
			return this._getStyles(a).backgroundColor
		},
		defaultBorderColor: function(a) {
			return this._getStyles(a).borderColor
		},
		getInnerPadding: function(a) {
			return this._getStyles(a).innerPadding
		},
		defaultShapeType: function(a) {
			return this._getStyles(a).shapeType
		}
	}
});
...
mindplot.NodeGraph = new Class({
	...
	setId: function(a) {
		$assert(typeof topic.getId() == "number", "id is not a number:" + a);
		this.getModel().setId(a)
	},
	_set2DElement: function(a) {
		this._elem2d = a
	},
	...
	setMouseEventsEnabled: function(a) {
		this._mouseEvents = a
	},
	...
	setOnFocus: function(a) {
		if (this._onFocus != a) {
			this._onFocus = a;
			var b = this.getOuterShape();
			if (a) {
				b.setFill(mindplot.Topic.OUTER_SHAPE_ATTRIBUTES_FOCUS.fillColor);
				b.setOpacity(1)
			} else {
				b.setFill(mindplot.Topic.OUTER_SHAPE_ATTRIBUTES.fillColor);
				b.setOpacity(0)
			}
			this.setCursor("move");
			this.closeEditors();
	...
	createDragNode: function(a) {
		var b = this._buildDragShape();
...
mindplot.NodeGraph.create = function(d, b) {
	$assert(d, "Model can not be null");
	var c = d.getType();
	$assert(c, "Node model type can not be null");
	var a;
	if (c == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
		a = new mindplot.CentralTopic(d, b)
	} else {
		if (c == mindplot.model.INodeModel.MAIN_TOPIC_TYPE) {
			a = new mindplot.MainTopic(d, b)
		} else {
			$assert(false, "unsupported node type:" + c)
		}
	}
	return a
};
mindplot.Topic = new Class({
	...
	_registerEvents: function() {
		...
		this.addEvent("dblclick", function(b) {
			a._getTopicEventDispatcher().show(a);
			b.stopPropagation()
		})
	},
	setShapeType: function(a) {
		this._setShapeType(a, true)
	},
	getParent: function() {
		return this._parent
	},
	_setShapeType: function(g, d) {
		var f = this.getModel();
		if ($defined(d) && d) {
			f.setShapeType(g)
		}
	...
	getShapeType: function() {
		...
		if (!$defined(a)) {
			a = mindplot.TopicStyle.defaultShapeType(this)
		}
		return a
	},
	...
	_buildShape: function(b, f) {
		$assert(b, "attributes can not be null");
		$assert(f, "shapeType can not be null");
		var a;
		if (f == mindplot.model.TopicShape.RECTANGLE) {
			a = new web2d.Rect(0, b)
		} else {
			if (f == mindplot.model.TopicShape.IMAGE) {
				var d = this.getModel();
				var c = d.getImageUrl();
				...
				a.getSize = function() {
					return d.getImageSize()
				};
				a.setPosition = function() {}
			} else {
				if (f == mindplot.model.TopicShape.ELLIPSE) {
					a = new web2d.Rect(0.9, b)
				} else {
					if (f == mindplot.model.TopicShape.ROUNDED_RECT) {
						a = new web2d.Rect(0.3, b)
					} else {
						if (f == mindplot.model.TopicShape.LINE) {
							a = new web2d.Line({
								strokeColor: "#495879",
								strokeWidth: 1
							});
							a.setSize = function(h, g) {
								this.size = {
									width: h,
									height: g
								};
							...
							$assert(false, "Unsupported figure shapeType:" + f)
						}
					}
				}
			}
	...
	getOuterShape: function() {
		if (!$defined(this._outerShape)) {
			var a = this._buildShape(mindplot.Topic.OUTER_SHAPE_ATTRIBUTES, mindplot.model.TopicShape.ROUNDED_RECT);
			a.setPosition(-2, -3);
			a.setOpacity(0);
			this._outerShape = a
		}
	...
	_buildIconGroup: function() {
		var h = this.getTextShape().getFontHeight();
	...
	addFeature: function(d) {
		var b = this.getOrBuildIconGroup();
		this.closeEditors();
		var c = this.getModel();
		c.addFeature(d);
	...
	removeFeature: function(c) {
		$assert(c, "featureModel could not be null");
		var b = this.getModel();
		b.removeFeature(c);
		var a = this.getIconGroup();
		if ($defined(a)) {
			a.removeIconByModel(c)
		}
		this._adjustShapes()
	},
	addRelationship: function(a) {
	...
	_buildTextShape: function(g) {
		var a = new web2d.Text();
	...
	getFontWeight: function() {
		...
		if (!$defined(a)) {
			var b = mindplot.TopicStyle.defaultFontStyle(this);
			a = b.weight
		}
		return a
	},
	...
	getText: function() {
		...
		var a = b.getText();
		if (!$defined(a)) {
			a = mindplot.TopicStyle.defaultText(this)
		}
		return a
	},
	setBackgroundColor: function(a) {
		this._setBackgroundColor(a, true)
	},
	_setBackgroundColor: function(b, e) {
		var d = this.getInnerShape();
		d.setFill(b);
	...
	getBackgroundColor: function() {
		...
		if (!$defined(a)) {
			a = mindplot.TopicStyle.defaultBackgroundColor(this)
		}
		return a
	},
	setBorderColor: function(a) {
		this._setBorderColor(a, true)
	},
	_setBorderColor: function(b, e) {
		var d = this.getInnerShape();
		d.setAttribute("strokeColor", b);
		var a = this.getShrinkConnector();
		if (a) {
			a.setAttribute("strokeColor", b)
		}
		if ($defined(e) && e) {
			var c = this.getModel();
			c.setBorderColor(b)
	...
	getBorderColor: function() {
		...
		if (!$defined(a)) {
			a = mindplot.TopicStyle.defaultBorderColor(this)
		}
		return a
	},
	...
	_registerDefaultListenersToElement: function(d, a) {
		var e = function(f) {
			if (a.isMouseEventsEnabled()) {
				a.handleMouseOver(f)
			}
		};
		...
		var b = this;
		d.addEvent("mousedown", function(g) {
			if (!b.isReadOnly()) {
				var h = true;
				if ((g.metaKey && Browser.Platform.mac) || (g.ctrlKey && !Browser.Platform.mac)) {
					h = !b.isOnFocus();
					g.stopPropagation();
					g.preventDefault()
				}
	...
	isCollapsed: function() {
		...
		var b = this.getParent();
		while (b && !a) {
			a = b.areChildrenShrunken();
			b = b.getParent()
		}
		return a
	},
	setChildrenShrunken: function(e) {
		var b = this.getModel();
	...
	showNoteEditor: function() {
		...
		var d = {
			getValue: function() {
				var f = a.findFeatureByType(mindplot.TopicFeature.Note.id);
				var e;
				if (f.length > 0) {
					e = f[0].getText()
				}
			...
			setValue: function(g) {
				...
				if (!$defined(g)) {
					...
					if (e.length > 0) {
						f.changeFeatureToTopic(c, e[0].getId(), {
							text: g
						})
					} else {
						f.addFeatureToTopic(c, mindplot.TopicFeature.Note.id, {
							text: g
						})
					}
				}
	...
	showLinkEditor: function() {
		...
		var d = {
			getValue: function() {
				var f = a.findFeatureByType(mindplot.TopicFeature.Link.id);
				var e;
				if (f.length > 0) {
					e = f[0].getUrl()
				}
			...
			setValue: function(g) {
				...
				if (!$defined(g)) {
					...
					if (e.length > 0) {
						...
						f.addFeatureToTopic(c, mindplot.TopicFeature.Link.id, {
							url: g
						})
					}
				}
	...
	setPosition: function(b) {
		$assert(b, "position can not be null");
		b.x = Math.ceil(b.x);
		b.y = Math.ceil(b.y);
		var c = this.getModel();
	...
	_updateConnectionLines: function() {
		...
		for (var a = 0; a < this._relationships.length; a++) {
			this._relationships[a].redraw()
		}
	},
	setBranchVisibility: function(b) {
		var c = this;
		var a = this;
		while (a != null && !a.isCentralTopic()) {
			c = a;
			a = c.getParent()
		}
		c.setVisibility(b)
	...
	moveToBack: function() {
		for (var b = 0; b < this._relationships.length; b++) {
			this._relationships[b].moveToBack()
		}
		var a = this.getShrinkConnector();
		if ($defined(a)) {
			a.moveToBack()
	...
	moveToFront: function() {
		...
		for (var b = 0; b < this._relationships.length; b++) {
			this._relationships[b].moveToFront()
		}
	},
	isVisible: function() {
		var a = this.get2DElement();
	...
	_setRelationshipLinesVisibility: function(a) {
		_.each(this._relationships, function(f) {
			var b = f.getSourceTopic();
	...
	_setTopicVisibility: function(d) {
	...
	_setChildrenVisibility: function(a) {
		var d = this.getChildren();
		var b = this.getModel();
	...
	setSize: function(b, e) {
		...
		if (a || e) {
			mindplot.NodeGraph.prototype.setSize.call(this, b);
			var c = this.getOuterShape();
	...
	_adjustShapes: function() {
		if (this._isInWorkspace) {
			...
			if (this.getShapeType() != mindplot.model.TopicShape.IMAGE) {
				...
				var i = this.getModel().getImageSize();
				this.setSize(i)
			}
		}
	},
	_flatten2DElements: function(b) {
		var a = [];
		var f = b.getChildren();
		for (var e = 0; e < f.length; e++) {
			...
			if (!g.areChildrenShrunken()) {
				var c = this._flatten2DElements(g);
				a = a.concat(c)
			}
		}
		return a
	},
	isChildTopic: function(e) {
		var a = (this.getId() == e.getId());
		if (!a) {
			var c = this.getChildren();
			for (var b = 0; b < c.length; b++) {
				var d = c[b];
...
mindplot.CentralTopic = new Class({
	...
	_registerEvents: function() {
		this.parent();
		this.addEvent("mousedown", function(a) {
			a.stopPropagation()
		})
	},
	...
	_updatePositionOnChangeSize: function() {
		var a = new core.Point(0, 0);
		this.setPosition(a)
	},
	getShrinkConnector: function() {
		return null
	},
	workoutOutgoingConnectionPoint: function(b) {
		$assert(b, "targetPoint can not be null");
...
mindplot.MainTopic = new Class({
	...
	_buildDragShape: function() {
		var f = this._buildShape(this.INNER_RECT_ATTRIBUTES, this.getShapeType());
		var a = this.getSize();
		...
		d.append(f);
		if (this.getShapeType() != mindplot.model.TopicShape.IMAGE) {
			var b = this._buildTextShape(true);
			var g = this.getText();
			b.setText(g);
			b.setOpacity(0.5);
			d.append(b)
	...
	_updatePositionOnChangeSize: function(c, a) {
		var b = Math.round((a.width - c.width) / 2);
		var d = this.getPosition();
		if ($defined(d)) {
			if (d.x > 0) {
				d.x = d.x + b
	...
	workoutIncomingConnectionPoint: function(a) {
		return mindplot.util.Shape.workoutIncomingConnectionPoint(this, a)
	},
	workoutOutgoingConnectionPoint: function(f) {
		$assert(f, "targetPoint can not be null");
		...
		if (this.getShapeType() == mindplot.model.TopicShape.LINE) {
			...
			a = mindplot.util.Shape.calculateRectConnectionPoint(h, d, g, true)
		}
		return a
	}
});
mindplot.DragTopic = new Class({
	...
	updateFreeLayout: function(c) {
		var b = (c.metaKey && Browser.Platform.mac) || (c.ctrlKey && !Browser.Platform.mac);
		if (this.isFreeLayoutOn() != b) {
	...
	connectTo: function(c) {
		$assert(c, "Parent connection node can not be null.");
	...
	applyChanges: function(c) {
		...
		if (!this.isFreeLayoutOn()) {
			...
			if (a) {
				var b = this.getConnectedToTopic();
				e = this._order;
				i = b
			}
			f.dragTopic(h, g, e, i)
...
mindplot.DragManager = new Class({
	...
	_buildMouseMoveListener: function(c, f, d) {
		var b = c.getScreenManager();
		var e = this;
		var a = function(i) {
			if (!e._isDragInProcess) {
				var g = d._listeners.startdragging;
				g(i, f);
				c.append(f);
	...
	_buildMouseUpListener: function(c, f, g, d) {
		var b = c.getScreenManager();
		var e = this;
		var a = function(i) {
			$assert(g.isDragTopic, "dragNode must be an DragTopic");
			...
			window.document.body.style.cursor = "default";
			if (e._isDragInProcess) {
				var h = d._listeners.enddragging;
				h(i, g);
...
mindplot.DragPivot = new Class({
	...
	getSize: function() {
		var a = this._getPivotRect();
		return a.getSize()
	},
	setVisibility: function(d) {
		if (this.isVisible() != d) {
			var c = this._getPivotRect();
			c.setVisibility(d);
	...
	removeFromWorkspace: function(c) {
		...
		if ($defined(this._straightLine)) {
			c.removeChild(this._straightLine)
		}
		if ($defined(this._curvedLine)) {
			c.removeChild(this._curvedLine)
		}
	},
	connectTo: function(a, g) {
		$assert(!this._outgoingLine, "Could not connect an already connected node");
	...
	disconnect: function(a) {
		$assert(a, "workspace can not be null.");
...
mindplot.ConnectionLine = new Class({
	initialize: function(c, f, e) {
		$assert(f, "parentNode node can not be null");
		...
		if (f.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			a = this._createLine(e, mindplot.ConnectionLine.CURVED);
			a.setSrcControlPoint(b[0]);
			a.setDestControlPoint(b[1])
		} else {
			a = this._createLine(e, mindplot.ConnectionLine.SIMPLE_CURVED);
			a.setSrcControlPoint(b[0]);
			a.setDestControlPoint(b[1])
		}
	...
	_getCtrlPoints: function(d, e) {
		var c = d.workoutOutgoingConnectionPoint(e.getPosition());
		var b = e.workoutIncomingConnectionPoint(d.getPosition());
		var a = (c.x - b.x) / 3;
		return [new core.Point(a, 0), new core.Point(-a, 0)]
	},
	_createLine: function(b, c) {
		if (!$defined(b)) {
			b = c
		}
		b = parseInt(b);
		...
		switch (b) {
			case mindplot.ConnectionLine.POLYLINE:
				a = new web2d.PolyLine();
				break;
			case mindplot.ConnectionLine.CURVED:
				a = new web2d.CurvedLine();
				break;
	...
	setVisibility: function(a) {
		this._line2d.setVisibility(a)
	},
	isVisible: function() {
		return this._line2d.isVisible()
	},
	setOpacity: function(a) {
		this._line2d.setOpacity(a)
	},
	redraw: function() {
		var g = this._line2d;
		...
		g.setFrom(h.x, h.y);
		g.setTo(b.x, b.y);
		if (g.getType() == "CurvedLine") {
			var d = this._getCtrlPoints(this._sourceTopic, this._targetTopic);
			g.setSrcControlPoint(d[0]);
			g.setDestControlPoint(d[1])
		}
		this._positionateConnector(c)
	},
	_positionateConnector: function(c) {
		var d = c.getPosition();
		var e = mindplot.Topic.CONNECTOR_WIDTH / 2;
		var g = c.getSize();
		var f, a;
		if (c.getShapeType() == mindplot.model.TopicShape.LINE) {
			f = g.height
		} else {
			f = g.height / 2
		}
		...
		if ($defined(b)) {
			if (Math.sign(d.x) > 0) {
				...
				a = -mindplot.Topic.CONNECTOR_WIDTH
			}
			b.setPosition(a, f)
		}
	},
	setStroke: function(a, c, b) {
		this._line2d.setStroke(null, null, a, b)
	},
	addToWorkspace: function(a) {
		a.append(this._line2d);
		this._line2d.moveToBack()
	},
	removeFromWorkspace: function(a) {
		a.removeChild(this._line2d)
	},
	getTargetTopic: function() {
		return this._targetTopic
	},
...
mindplot.Relationship = new Class({
	...
	initialize: function(e, g, d) {
		$assert(e, "sourceNode can not be null");
		...
		if ($defined(d.getSrcCtrlPoint())) {
			var b = d.getSrcCtrlPoint().clone();
			this.setSrcControlPoint(b)
		}
		if ($defined(d.getDestCtrlPoint())) {
			var a = d.getDestCtrlPoint().clone();
	...
	_positionArrows: function() {
		...
		if (this._line2d.getType() == "CurvedLine") {
			...
			if (this._endArrow) {
				this._endArrow.setControlPoint(a[1])
			}
		} else {
	...
	removeFromWorkspace: function(a) {
		a.removeChild(this._focusShape);
	...
	setOpacity: function(a) {
		this.parent(a);
		if (this._showEndArrow) {
			this._endArrow.setOpacity(a)
		}
		if (this._showStartArrow) {
			this._startArrow.setOpacity(a)
		}
	},
	setShowEndArrow: function(a) {
		this._showEndArrow = a;
		if (this._isInWorkspace) {
			this.redraw()
		}
	},
	setShowStartArrow: function(a) {
		this._showStartArrow = a;
		if (this._isInWorkspace) {
			this.redraw()
		}
	},
	setFrom: function(a, b) {
		$assert($defined(a), "x must be defined");
	...
	setTo: function(a, b) {
		...
		if (this._endArrow) {
			this._endArrow.setFrom(a, b)
		}
	},
	setSrcControlPoint: function(a) {
		this._line2d.setSrcControlPoint(a);
		this._startArrow.setControlPoint(a)
	},
	setDestControlPoint: function(a) {
		this._line2d.setDestControlPoint(a);
		if (this._showEndArrow) {
			this._endArrow.setControlPoint(a)
		}
	},
	getControlPoints: function() {
	...
	setIsSrcControlPointCustom: function(a) {
		this._line2d.setIsSrcControlPointCustom(a)
	},
	setIsDestControlPointCustom: function(a) {
		this._line2d.setIsDestControlPointCustom(a)
	},
	getId: function() {
		return this._model.getId()
	},
...
mindplot.DragConnector = new Class({
	initialize: function(a, b) {
		$assert(a, "designerModel can not be null");
		$assert(b, "workspace can not be null");
		this._designerModel = a;
		this._workspace = b
	},
	checkConnection: function(a) {
		var d = this._designerModel.getTopics();
		var c = this._searchConnectionCandidates(a);
		var b = a.getConnectedToTopic();
		if (b && (c.length == 0 || c[0] != b)) {
			a.disconnect(this._workspace)
		}
		if (!a.isConnected() && c.length > 0) {
			a.connectTo(c[0])
		}
	},
	_searchConnectionCandidates: function(c) {
		var h = this._designerModel.getTopics();
	...
	_proximityWeight: function(a, d, b, c) {
		var e = d.getPosition();
		return (a ? 0 : 200) + Math.abs(e.x - b.x) + Math.abs(e.y - b.y) + (c == d ? 0 : 100)
	},
	_isVerticallyAligned: function(c, b, a) {
		return Math.abs(a.y - b.y) < c.height / 2
	}
});
		...
		d.addEvent("keydown", function(f) {
			switch (f.key) {
				...
				default:
					...
					c.size = e;
					if (a.offsetWidth > (parseInt(d.style.width) - 100)) {
						d.style.width = (a.offsetWidth + 100) + "px"
					}
					break
			}
			f.stopPropagation()
...
mindplot.MultilineTextEditor = new Class({
	...
	_buildEditor: function() {
		var a = $("<div></div>").attr("id", "textContainer").css({
			display: "none",
			zIndex: "8",
			overflow: "hidden",
			border: "0 none"
		});
		var b = $('<textarea tabindex="-1" value="" wrap="off" ></textarea>').css({
	...
	_registerEvents: function(a) {
		...
		c.on("keydown", function(i) {
			switch (jQuery.hotkeys.specialKeys[i.keyCode]) {
				case "esc":
					b.close(false);
					break;
				case "enter":
					if (i.metaKey || i.ctrlKey) {
						...
						var g = "";
						if (d < j.length) {
							g = j.substring(d, j.length)
						}
						c.val(h + "\n" + g);
						if (c[0].setSelectionRange) {
							c.focus();
...
mindplot.TextEditorFactory.getTextEditorFromName = function(b) {
	var a = null;
	if (b == "RichTextEditor") {
		a = mindplot.RichTextEditor
	} else {
		a = mindplot.TextEditor
	}
	return a
};
mindplot.util.Shape = {
	isAtRight: function(a, b) {
		$assert(a, "Source can not be null");
		$assert(b, "Target can not be null");
		return a.x < b.x
	},
	calculateRectConnectionPoint: function(c, b, e) {
		$assert(c, "rectCenterPoint can  not be null");
		$assert(b, "rectSize can  not be null");
		$assert($defined(e), "isRight can  not be null");
		var a = new core.Point();
		var d = 2;
		if (e) {
			a.setValue(c.x - (b.width / 2) + d, c.y)
		} else {
			a.setValue(parseFloat(c.x) + (b.width / 2) - d, c.y)
		}
		return a
	},
	calculateRelationShipPointCoordinates: function(d, g) {
		var k = d.getSize();
		var e = d.getPosition();
		var b;
		var a = e.y - g.y;
	...
	calculateDefaultControlPoints: function(f, d) {
		var j = f.y - d.y;
		var n = f.x - d.x;
	...
	workoutIncomingConnectionPoint: function(d, c) {
		$assert(c, "sourcePoint can not be null");
		var g = d.getPosition();
		var b = d.getSize();
		var f = mindplot.util.Shape.isAtRight(c, g);
		var a = mindplot.util.Shape.calculateRectConnectionPoint(g, b, f);
		if (d.getShapeType() == mindplot.model.TopicShape.LINE) {
			a.y = a.y + (d.getSize().height / 2)
		}
		var e = mindplot.Topic.CONNECTOR_WIDTH / 2;
		if (!f) {
			a.x = a.x + e
...
mindplot.util.FadeEffect = new Class({
	...
	start: function() {
		var a = this._isVisible;
		_.each(this._element, function(b) {
			if (b) {
				b.setVisibility(a)
			}
		});
...
mindplot.persistence.XMLSerializer_Pela = new Class({
	toXML: function(o) {
		$assert(o, "Can not save a null mindmap");
		var m = core.Utils.createDocument();
		var n = m.createElement("map");
		var b = o.getId();
		if ($defined(b)) {
			n.setAttribute("name", this.rmXmlInv(b))
		}
		var l = o.getVersion();
		if ($defined(l)) {
			n.setAttribute("version", l)
		}
		m.appendChild(n);
		var a = o.getBranches();
		...
		if (k.length > 0) {
			for (var e = 0; e < k.length; e++) {
				var h = k[e];
				if (o.findNodeById(h.getFromNode()) !== null && o.findNodeById(h.getToNode()) !== null) {
					var c = this._relationshipToXML(m, h);
					n.appendChild(c)
				}
			}
		}
	...
	_topicToXML: function(y, A) {
		var v = y.createElement("topic");
		if (A.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			v.setAttribute("central", "true")
		} else {
			var p = A.getPosition();
			v.setAttribute("position", p.x + "," + p.y);
			var C = A.getOrder();
			if (typeof C === "number" && isFinite(C)) {
				v.setAttribute("order", C)
			}
		}
		var u = A.getText();
		if ($defined(u)) {
			this._noteTextToXML(y, v, u)
		}
		var b = A.getShapeType();
		if ($defined(b)) {
			v.setAttribute("shape", b);
			if (b == mindplot.model.TopicShape.IMAGE) {
				v.setAttribute("image", A.getImageSize().width + "," + A.getImageSize().height + ":" + A.getImageUrl())
			}
		}
		if (A.areChildrenShrunken() && A.getType() != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			v.setAttribute("shrink", "true")
		}
		var x = A.getId();
		v.setAttribute("id", x);
		var w = "";
		...
		w += (h ? h : "") + ";";
		if ($defined(E) || $defined(f) || $defined(c) || $defined(s) || $defined(h)) {
			v.setAttribute("fontStyle", w)
		}
		var o = A.getBackgroundColor();
		if ($defined(o)) {
			v.setAttribute("bgColor", o)
		}
		var e = A.getBorderColor();
		if ($defined(e)) {
			v.setAttribute("brColor", e)
		}
		var t = A.getMetadata();
		if ($defined(t)) {
			v.setAttribute("metadata", t)
		}
		var n = A.getFeatures();
		for (var D = 0; D < n.length; D++) {
			...
			for (var F in r) {
				var z = r[F];
				if (F == "text") {
					var a = y.createCDATASection(this.rmXmlInv(z));
					d.appendChild(a)
				} else {
					d.setAttribute(F, this.rmXmlInv(z))
				}
			}
			v.appendChild(d)
		}
	...
	_noteTextToXML: function(a, c, e) {
		if (e.indexOf("\n") == -1) {
			c.setAttribute("text", this.rmXmlInv(e))
		} else {
	...
	_relationshipToXML: function(d, f) {
		var c = d.createElement("relationship");
		c.setAttribute("srcTopicId", f.getFromNode());
		c.setAttribute("destTopicId", f.getToNode());
		var e = f.getLineType();
		c.setAttribute("lineType", e);
		if (e == mindplot.ConnectionLine.CURVED || e == mindplot.ConnectionLine.SIMPLE_CURVED) {
			if ($defined(f.getSrcCtrlPoint())) {
				var b = f.getSrcCtrlPoint();
				c.setAttribute("srcCtrlPoint", Math.round(b.x) + "," + Math.round(b.y))
			}
			if ($defined(f.getDestCtrlPoint())) {
				var a = f.getDestCtrlPoint();
	...
	loadFromDom: function(d, k) {
		$assert(d, "dom can not be null");
		...
		for (var e = 0; e < b.length; e++) {
			...
			if (a.nodeType == 1) {
				switch (a.tagName) {
					case "topic":
						var f = this._deserializeNode(a, j);
						j.addBranch(f);
						break;
					case "relationship":
						var g = this._deserializeRelationship(a, j);
						if (g != null) {
							j.addRelationship(g)
						}
						break
	...
	_deserializeNode: function(b, a) {
		var g = (b.getAttribute("central") != null) ? mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE : mindplot.model.INodeModel.MAIN_TOPIC_TYPE;
		...
		if ($defined(d)) {
			B.setShapeType(d);
			if (d == mindplot.model.TopicShape.IMAGE) {
				var x = b.getAttribute("image");
		...
		if ($defined(D) && D != "NaN") {
			B.setOrder(parseInt(D))
		}
		var H = b.getAttribute("shrink");
		if ($defined(H) && g != mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			B.setChildrenShrunken(H)
		}
		...
		for (var F = 0; F < h.length; F++) {
			...
			if (o.nodeType == Node.ELEMENT_NODE) {
				if (o.tagName == "topic") {
					var e = this._deserializeNode(o, a);
					e.connectTo(B)
				} else {
					if (mindplot.TopicFeature.isSupported(o.tagName)) {
						var E = o.attributes;
						var q = {};
						for (var C = 0; C < E.length; C++) {
							var r = E.item(C);
						...
						if (o.tagName == "text") {
							var u = this._deserializeNodeText(o);
							B.setText(u)
						}
					}
				}
	...
	_deserializeTextAttr: function(c) {
		var d = c.getAttribute("text");
		if (!$defined(d)) {
			var b = c.childNodes;
			for (var a = 0; a < b.length; a++) {
				var e = b[a];
				if (e.nodeType == Node.CDATA_SECTION_NODE) {
					d = e.nodeValue
				}
			}
		} else {
	...
	_deserializeNodeText: function(c) {
		var b = c.childNodes;
		var d = null;
		for (var a = 0; a < b.length; a++) {
			var e = b[a];
	...
	_deserializeRelationship: function(e, i) {
		var a = e.getAttribute("srcTopicId");
		...
		if (i.findNodeById(a) == null || i.findNodeById(d) == null) {
			return null
		}
		var f = i.createRelationship(a, d);
		f.setLineType(g);
		if ($defined(c) && c != "") {
			f.setSrcCtrlPoint(core.Point.fromString(c))
		}
		if ($defined(j) && j != "") {
			f.setDestCtrlPoint(core.Point.fromString(j))
		}
		f.setEndArrow("false");
	...
	rmXmlInv: function(d) {
		...
		for (var b = 0; b < d.length; b++) {
			...
			if ((e == 9) || (e == 10) || (e == 13) || ((e >= 32) && (e <= 55295)) || ((e >= 57344) && (e <= 65533)) || ((e >= 65536) && (e <= 1114111))) {
				a = a + d.charAt(b)
			}
		}
		return a
...
mindplot.persistence.Pela2TangoMigrator = new Class({
	initialize: function(a) {
		this._pelaSerializer = a;
		this._tangoSerializer = new mindplot.persistence.XMLSerializer_Tango()
	},
	toXML: function(a) {
		return this._tangoSerializer.toXML(a)
	},
	loadFromDom: function(c, b) {
		$assert($defined(b), "mapId can not be null");
	...
	_fixOrder: function(b) {
		var f = b.getBranches()[0];
		var e = f.getChildren();
		var d = [];
		var g = [];
		for (var c = 0; c < e.length; c++) {
	...
	_fixPosition: function(b) {
		var e = b.getBranches()[0];
		var d = e.getChildren();
		for (var c = 0; c < d.length; c++) {
			var f = d[c];
			var a = f.getPosition();
...
mindplot.persistence.XMLSerializer_Beta = new Class({
	toXML: function(b) {
		$assert(b, "Can not save a null mindmap");
	...
	_topicToXML: function(v, w) {
		var t = v.createElement("topic");
		if (w.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			t.setAttribute("central", true)
		} else {
			var p = w.getParent();
			if (p == null || p.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
				var m = w.getPosition();
		...
		var s = w.getText();
		if ($defined(s)) {
			t.setAttribute("text", s)
		}
		var a = w.getShapeType();
		if ($defined(a)) {
			t.setAttribute("shape", a)
		}
		if (w.areChildrenShrunken()) {
			t.setAttribute("shrink", true)
		}
		var u = "";
		...
		u += (h ? h : "") + ";";
		if ($defined(B) || $defined(f) || $defined(c) || $defined(r) || $defined(h)) {
			t.setAttribute("fontStyle", u)
		}
		var l = w.getBackgroundColor();
		if ($defined(l)) {
			t.setAttribute("bgColor", l)
		}
		var e = w.getBorderColor();
		if ($defined(e)) {
			t.setAttribute("brColor", e)
		}
		var z;
		var D = w.getIcons();
	...
	_iconToXML: function(a, c) {
		var b = a.createElement("icon");
		b.setAttribute("id", c.getIconType());
		return b
	},
	_linkToXML: function(a, b) {
		var c = a.createElement("link");
		c.setAttribute("url", b.getUrl());
		return c
	},
	_noteToXML: function(a, c) {
		var b = a.createElement("note");
		b.setAttribute("text", c.getText());
		return b
	},
	loadFromDom: function(c, j) {
		$assert(c, "Dom can not be null");
		...
		for (var d = 0; d < b.length; d++) {
			var a = b[d];
			if (a.nodeType == 1) {
				var e = this._deserializeNode(a, g);
				g.addBranch(e)
			}
		}
		g.setId(j);
	...
	_deserializeNode: function(b, a) {
		var h = (b.getAttribute("central") != null) ? mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE : mindplot.model.INodeModel.MAIN_TOPIC_TYPE;
		...
		for (var s = 0; s < j.length; s++) {
			var m = j[s];
			if (m.nodeType == 1) {
				$assert(m.tagName == "topic" || m.tagName == "icon" || m.tagName == "link" || m.tagName == "note", "Illegal node type:" + m.tagName);
				if (m.tagName == "topic") {
					var f = this._deserializeNode(m, a);
					f.connectTo(q)
				} else {
					if (m.tagName == "icon") {
						var t = this._deserializeIcon(m, q);
						q.addFeature(t)
					} else {
						if (m.tagName == "link") {
							var n = this._deserializeLink(m, q);
							q.addFeature(n)
						} else {
							if (m.tagName == "note") {
								var e = this._deserializeNote(m, q);
								q.addFeature(e)
							}
						}
					}
...
mindplot.persistence.Beta2PelaMigrator = new Class({
	initialize: function(a) {
		this._betaSerializer = a;
		this._pelaSerializer = new mindplot.persistence.XMLSerializer_Pela()
	},
	toXML: function(a) {
		return this._pelaSerializer.toXML(a)
	},
	loadFromDom: function(e, c) {
		$assert($defined(c), "mapId can not be null");
	...
	_fixPosition: function(a) {
		var d = a.getPosition();
		var c = d.x > 0;
		var b = this;
		_.each(a.getChildren(), function(e) {
			if (!e.getPosition()) {
				e.setPosition(d.x + (50 * c ? 1 : -1), d.y)
			}
			b._fixPosition(e)
		})
	}
...
mindplot.persistence.XMLSerializerFactory.getSerializerFromMindmap = function(a) {
	return mindplot.persistence.XMLSerializerFactory.getSerializer(a.getVersion())
};
mindplot.persistence.XMLSerializerFactory.getSerializerFromDocument = function(b) {
	var a = b.documentElement;
	return mindplot.persistence.XMLSerializerFactory.getSerializer(a.getAttribute("version"))
};
mindplot.persistence.XMLSerializerFactory.getSerializer = function(b) {
	if (!$defined(b)) {
		b = mindplot.persistence.ModelCodeName.BETA
	}
...
mindplot.PersistenceManager = new Class({
	Static: {
		loadFromDom: function(b, a) {
			$assert(b, "mapId can not be null");
	...
	initialize: function() {},
	save: function(h, c, i, k, d) {
		$assert(h, "mindmap can not be null");
	...
	load: function(a) {
		$assert(a, "mapId can not be null");
	...
	loadMapDom: function(a) {
		throw new Error("Method must be implemented")
	},
	saveMapXml: function(c, e, a, f, b, d) {
		throw new Error("Method must be implemented")
	},
	unlockMap: function(a) {
		throw new Error("Method must be implemented")
	}
});
mindplot.PersistenceManager.init = function(a) {
	mindplot.PersistenceManager._instance = a
};
...
mindplot.RESTPersistenceManager = new Class({
	...
	saveMapXml: function(g, a, i, f, h, d) {
		var b = {
			id: g,
			xml: a,
			properties: i
		...
		if (!e.onSave) {
			e.onSave = true;
			e.clearTimeout = setTimeout(function() {
				e.clearTimeout = null;
				e.onSave = false
			}, 10000);
			$.ajax({
				url: this.documentUrl.replace("{id}", g) + "?" + c,
				type: "put",
				dataType: "json",
				...
				fail: function(n, p) {
					...
					if (o != null && o.indexOf("application/json") != -1) {
						var k = null;
						try {
							k = $.parseJSON(l);
	...
	discardChanges: function(a) {
		$.ajax({
			url: this.revertUrl.replace("{id}", a),
			async: false,
			method: "post",
			headers: {
	...
	_buildError: function(c) {
		var b = c ? c.globalErrors[0] : null;
		var a = c ? c.globalSeverity : null;
		if (!b) {
			b = $msg("SAVE_COULD_NOT_BE_COMPLETED")
		}
		if (!a) {
			a = "INFO"
		}
	...
	loadMapDom: function(b) {
		var a;
		$.ajax({
			url: this.documentUrl.replace("{id}", b) + "/xml",
			method: "get",
			async: false,
			headers: {
				"Content-Type": "text/plain",
...
mindplot.LocalStorageManager = new Class({
	...
	saveMapXml: function(c, d, a, e, b) {
		localStorage.setItem(c + "-xml", d)
	},
	discardChanges: function(a) {
		localStorage.removeItem(a + "-xml")
	},
	loadMapDom: function(b) {
		var a = localStorage.getItem(b + "-xml");
		if (a == null || this.forceLoad) {
			$.ajax({
				url: this.documentUrl.replace("{id}", b),
				headers: {
					"Content-Type": "text/plain",
					Accept: "application/xml"
				},
...
mindplot.IconGroup = new Class({
	initialize: function(b, a) {
		$assert($defined(b), "topicId can not be null");
	...
	addIcon: function(b, a) {
		$defined(b, "icon is not defined");
		b.setGroup(this);
		this._icons.push(b);
		this._resize(this._icons.length);
		...
		if (a) {
			this._removeTip.decorate(this._topicId, b)
		}
	},
	_findIconFromModel: function(b) {
		var a = null;
		_.each(this._icons, function(d) {
			var c = d.getModel();
			if (c.getId() == b.getId()) {
				a = d
			}
		...
		if (a == null) {
			throw new Error("Icon can no be found:" + b.getId() + ", Icons:" + this._icons)
		}
		return a
	},
	removeIconByModel: function(b) {
		$assert(b, "featureModel can not be null");
		var a = this._findIconFromModel(b);
		this._removeIcon(a)
	},
	_removeIcon: function(a) {
		$assert(a, "icon can not be null");
		this._removeTip.close(0);
		this._group.removeChild(a.getImage());
		this._icons.erase(a);
		this._resize(this._icons.length);
		var b = this;
		_.each(this._icons, function(d, c) {
			b._positionIcon(d, c)
		})
	},
	moveToFront: function() {
	...
	_registerListeners: function() {
		this._group.addEvent("click", function(a) {
			a.stopPropagation()
		});
		this._group.addEvent("dblclick", function(a) {
			a.stopPropagation()
		})
	},
	_resize: function(b) {
		this._group.setSize(b * this._iconSize.width, this._iconSize.height);
		var a = mindplot.Icon.SIZE + (mindplot.IconGroup.ICON_PADDING * 2);
		this._group.setCoordSize(b * a, a)
	},
	_positionIcon: function(c, b) {
		var a = mindplot.Icon.SIZE + (mindplot.IconGroup.ICON_PADDING * 2);
...
mindplot.IconGroup.RemoveTip = new Class({
	initialize: function(a) {
		$assert(a, "group can not be null");
		this._fadeElem = a
	},
	show: function(b, a) {
		$assert(a, "icon can not be null");
		if (this._activeIcon != a) {
			if (this._activeIcon) {
				this.close(0)
			}
			var e = a.getPosition();
	...
	close: function(a) {
		...
		if (this._activeIcon) {
			...
			if (!$defined(a) || a == 0) {
				...
				this._closeTimeoutId = d.delay(a)
			}
		}
	},
	_buildWeb2d: function() {
		var b = new web2d.Group({
			width: 10,
			height: 10,
			x: 0,
			y: 0,
		...
		b.append(e);
		var c = new web2d.Line({
			stroke: "1 solid white"
		});
		c.setFrom(1, 1);
		c.setTo(9, 9);
		b.append(c);
		var a = new web2d.Line({
			stroke: "1 solid white"
		});
		a.setFrom(1, 9);
		a.setTo(9, 1);
...
mindplot.LinkIcon = new Class({
	...
	_registerEvents: function() {
		...
		if (!this._readOnly) {
			...
			this.addEvent("mouseleave", function(b) {
...
mindplot.NoteIcon = new Class({
	...
	_registerEvents: function() {
		...
		if (!this._readOnly) {
			this.addEvent("click", function(b) {
				a._topic.showNoteEditor();
				b.stopPropagation()
			})
		}
		this._tip = new mindplot.widget.FloatingTip($(a.getImage()._peer._native), {
			title: $msg("NOTE"),
			container: "body",
			content: function() {
				return a._buildTooltipContent()
			},
			html: true,
			placement: "bottom",
			destroyOnExit: true
	...
	_buildTooltipContent: function() {
		if ($("body").find("#textPopoverNote").length == 1) {
			var b = $("body").find("#textPopoverNote");
			b.text(this._linksModel.getText())
		} else {
			var a = $('<div id="textPopoverNote"></div>').css({
				padding: "5px"
			});
			var b = $("<div></div>").text(this._linksModel.getText()).css({
...
mindplot.ActionIcon = new Class({
	...
	addEvent: function(b, a) {
		this.getImage().addEvent(b, a)
	},
	addToGroup: function(a) {
		a.append(this.getImage())
	},
	setVisibility: function(a) {
		this.getImage().setVisibility(a)
	},
	isVisible: function() {
		return this.getImage().isVisible()
	},
	setCursor: function(a) {
		return this.getImage().setCursor(a)
	},
	moveToBack: function(a) {
		return this.getImage().moveToBack(a)
	},
	moveToFront: function(a) {
		return this.getImage().moveToFront(a)
	}
});
mindplot.ImageIcon = new Class({
	Extends: mindplot.Icon,
	initialize: function(b, d, g) {
		$assert(d, "iconModel can not be null");
		$assert(b, "topic can not be null");
	...
	_getNextFamilyIconId: function(c) {
		var d = this._getFamilyIcons(c);
		$assert(d != null, "Family Icon not found!");
		var a = null;
		for (var b = 0; b < d.length && a == null; b++) {
			if (d[b] == c) {
				if (b == (d.length - 1)) {
					a = d[0]
				} else {
	...
	_getFamilyIcons: function(e) {
		$assert(e != null, "id must not be null");
		$assert(e.indexOf("_") != -1, "Invalid icon id (it must contain '_')");
		var a = null;
		for (var b = 0; b < mindplot.ImageIcon.prototype.ICON_FAMILIES.length; b++) {
			var d = mindplot.ImageIcon.prototype.ICON_FAMILIES[b];
...
mindplot.model.FeatureModel = new Class({
	...
	initialize: function(a) {
		...
		this._type = a;
		this._attributes = {};
		this["is" + $.camelCase(a) + "Model"] = function() {
			return true
		}
	},
	...
	setAttributes: function(a) {
		for (key in a) {
			this["set" + key.capitalize()](a[key])
		}
	},
	setAttribute: function(a, b) {
		$assert(a, "key id can not be null");
...
mindplot.model.IconModel = new Class({
	...
	setIconType: function(a) {
		$assert(a, "iconType id can not be null");
...
mindplot.model.LinkModel = new Class({
	...
	_fixUrl: function(b) {
		var a = b;
		if (!a.contains("http://") && !a.contains("https://") && !a.contains("mailto://")) {
			a = "http://" + a
		}
		return a
	},
	setUrlType: function(a) {
		$assert(a, "urlType can not be null");
...
mindplot.DesignerActionRunner = new Class({
	...
	execute: function(a) {
		$assert(a, "command can not be null");
	...
	fireChangeEvent: function() {
		var a = this._undoManager.buildEvent();
...
mindplot.DesignerUndoManager = new Class({
	...
	enqueue: function(c) {
		$assert(c, "Command can  not be null");
		var b = this._undoQueue.length;
		if (c.discardDuplicated && b > 0) {
			var a = this._undoQueue[b - 1];
			if (a.discardDuplicated != c.discardDuplicated) {
	...
	markAsChangeBase: function() {
		var a = this._undoQueue.length;
		if (a > 0) {
			var b = this._undoQueue[a - 1];
	...
	hasBeenChanged: function() {
		var a = true;
		var b = this._undoQueue.length;
		if (b == 0 && this._baseId == 0) {
			a = false
		} else {
			if (b > 0) {
				var c = this._undoQueue[b - 1];
...
mindplot.ControlPoint = new Class({
	initialize: function() {
		...
		var c = this;
		this._controlPointsController[0].addEvent("mousedown", function(d) {
			(c._mouseDown)(d, mindplot.ControlPoint.FROM, c)
		});
		this._controlPointsController[0].addEvent("click", function(d) {
			(c._mouseClick)(d)
		});
		this._controlPointsController[0].addEvent("dblclick", function(d) {
			(c._mouseClick)(d)
		});
		this._controlPointsController[1].addEvent("mousedown", function(d) {
			(c._mouseDown)(d, mindplot.ControlPoint.TO, c)
		});
		this._controlPointsController[1].addEvent("click", function(d) {
			(c._mouseClick)(d)
		});
		this._controlPointsController[1].addEvent("dblclick", function(d) {
			(c._mouseClick)(d)
		})
	},
	setLine: function(a) {
	...
	_mouseDown: function(c, a, b) {
		if (!this._isBinded) {
			this._isBinded = true;
			this._mouseMoveFunction = function(d) {
				(b._mouseMoveEvent)(d, a, b)
			};
			this._workspace.getScreenManager().addEvent("mousemove", this._mouseMoveFunction);
			this._mouseUpFunction = function(d) {
				(b._mouseUp)(d, a, b)
			};
	...
	_mouseMoveEvent: function(c, a) {
		var b = this._workspace.getScreenManager();
		var e = b.getWorkspaceMousePosition(c);
		var d;
		if (a == 0) {
			d = mindplot.util.Shape.calculateRelationShipPointCoordinates(this._line.getSourceTopic(), e);
			this._line.setFrom(d.x, d.y);
			this._line.setSrcControlPoint(new core.Point(e.x - d.x, e.y - d.y))
		} else {
			d = mindplot.util.Shape.calculateRelationShipPointCoordinates(this._line.getTargetTopic(), e);
	...
	getControlPoint: function(a) {
		return this._controls[a]
	},
	getOriginalEndPoint: function(a) {
		return this._endPoint[a]
	},
	getOriginalCtrlPoint: function(a) {
		return this._orignalCtrlPoint[a]
	}
});
...
mindplot.RelationshipPivot = new Class({
	...
	start: function(b, e) {
		$assert(b, "sourceTopic can not be null");
		...
		if (b != null) {
			...
			var g = c.getTopics();
			_.each(g, function(h) {
				h.addEvent("ontfocus", this._onTopicClick)
			}.bind(this))
		}
	},
	dispose: function() {
		var a = this._workspace;
		if (this._isActive()) {
			a.removeEvent("mousemove", this._mouseMoveEvent);
			a.removeEvent("click", this._onClickEvent);
			var b = this._designer.getModel();
			var d = b.getTopics();
			var c = this;
			_.each(d, function(e) {
				e.removeEvent("ontfocus", c._onTopicClick)
			});
	...
	_mouseMove: function(f) {
		var c = this._workspace.getScreenManager();
	...
	_cleanOnMouseClick: function(a) {
		this.dispose();
		a.stopPropagation()
	},
	_calculateFromPosition: function(d) {
		var b = this._sourceTopic.getPosition();
		if (this._sourceTopic.getType() == mindplot.model.INodeModel.CENTRAL_TOPIC_TYPE) {
			b = mindplot.util.Shape.workoutIncomingConnectionPoint(this._sourceTopic, d)
		}
	...
	_connectOnFocus: function(e, d) {
		var a = this._sourceTopic;
		var b = this._designer.getMindmap();
		if (d.getId() != a.getId()) {
			var c = b.createRelationship(d.getId(), a.getId());
...
mindplot.TopicFeature = {
	...
	isSupported: function(a) {
		return mindplot.TopicFeature._featuresMetadataById.some(function(b) {
			return b.id == a
		})
	},
	createModel: function(c, a) {
		$assert(c, "type can not be null");
		$assert(a, "attributes can not be null");
		var b = mindplot.TopicFeature._featuresMetadataById.filter(function(d) {
			return d.id == c
		})[0].model;
		return new b(a)
	},
	createIcon: function(b, a, d) {
		$assert(b, "topic can not be null");
		$assert(a, "model can not be null");
		var c = mindplot.TopicFeature._featuresMetadataById.filter(function(e) {
			return e.id == a.getType()
		})[0].icon;
		return new c(b, a, d)
	}
};
...
mindplot.commands.GenericFunctionCommand = new Class({
	...
	initialize: function(c, a, b) {
		$assert(c, "commandFunc must be defined");
	...
	execute: function(a) {
		if (!this.applied) {
			...
			if (d != null) {
				var b = this;
				_.each(d, function(f) {
					var e = b._commandFunc(f, b._value);
					b._oldValues.push(e)
				})
			}
			this.applied = true
	...
	undoExecute: function(a) {
		if (this.applied) {
			var c = a.findTopics(this._topicsId);
			var b = this;
			_.each(c, function(e, d) {
				b._commandFunc(e, b._oldValues[d])
			});
			this.applied = false;
			this._oldValues = []
		} else {
...
mindplot.commands.DeleteCommand = new Class({
	Extends: mindplot.Command,
	initialize: function(b, a) {
		$assert($defined(a), "topicIds can not be null");
		this.parent();
		this._relIds = a;
		this._topicIds = b;
	...
	execute: function(b) {
		var c = this._filterChildren(this._topicIds, b);
		if (c.length > 0) {
			_.each(c, function(g) {
				g.closeEditors();
				var f = g.getModel();
				var h = this._collectInDepthRelationships(g);
				this._deletedRelModel.append(h.map(function(j) {
					return j.getModel().clone()
				}));
				_.each(h, function(j) {
					b.deleteRelationship(j)
				});
				var e = f.clone();
		...
		if (a.length > 0) {
			_.each(a, function(d) {
				this._deletedRelModel.push(d.getModel().clone());
				b.deleteRelationship(d)
			}, this)
		}
	},
	undoExecute: function(c) {
		_.each(this._deletedTopicModels, function(d) {
			c.createTopic(d)
		}, this);
		_.each(this._deletedTopicModels, function(f, e) {
			var h = c.findTopics(f.getId());
			var g = this._parentTopicIds[e];
			if (g) {
				var d = c.findTopics(g);
				c.connect(h[0], d[0])
			}
		}, this);
		_.each(this._deletedRelModel, function(d) {
			c.addRelationship(d)
		});
		_.each(this._deletedTopicModels, function(d) {
			var e = c.findTopics(d.getId());
			e[0].setBranchVisibility(true)
		}, this);
		if (this._deletedTopicModels.length > 0) {
			var a = this._deletedTopicModels[0];
	...
	_filterChildren: function(d, b) {
		var c = b.findTopics(d);
		var a = [];
		_.each(c, function(e) {
			var f = e.getParent();
			var g = false;
			while (f != null && !g) {
				g = d.contains(f.getId());
				if (g) {
					break
				}
				f = f.getParent()
	...
	_collectInDepthRelationships: function(d) {
		var a = [];
		a.append(d.getRelationships());
		var f = d.getChildren();
		var c = f.map(function(g) {
			return this._collectInDepthRelationships(g)
		}, this);
		a.append(c.flatten());
		if (a.length > 0) {
			a = a.sort(function(h, g) {
				return h.getModel().getId() - g.getModel().getId()
			});
			var b = [a[0]];
...
mindplot.commands.DragTopicCommand = new Class({
	...
	initialize: function(d, b, a, c) {
		$assert(d, "topicId must be defined");
		this._topicsId = d;
		if ($defined(c)) {
			this._parentId = c.getId()
		}
		this.parent();
		this._position = b;
		this._order = a
	...
	execute: function(e) {
		var c = e.findTopics(this._topicsId)[0];
		c.setVisibility(false);
		var b = c.getOutgoingConnectedTopic();
		var d = c.getOrder();
		var f = c.getPosition();
		if ($defined(b) && b != this._parentId) {
			e.disconnect(c)
		}
		if (this._order != null) {
			c.setOrder(this._order)
		} else {
			if (this._position != null) {
				e.moveTopic(c, this._position)
			} else {
				$assert("Illegal command state exception.")
			}
		} if (b != this._parentId) {
			if ($defined(this._parentId)) {
				var a = e.findTopics(this._parentId)[0];
				e.connect(c, a)
			}
			this._parentId = null;
			if ($defined(b)) {
				this._parentId = b.getId()
			}
		}
...
mindplot.commands.AddTopicCommand = new Class({
	Extends: mindplot.Command,
	initialize: function(b, a) {
		$assert(b, "models can not be null");
	...
	execute: function(a) {
		var b = this;
		_.each(this._models, function(f, e) {
			var d = a.createTopic(f);
			if (b._parentsIds) {
				var h = b._parentsIds[e];
				if ($defined(h)) {
					var c = a.findTopics(h)[0];
					a.connect(d, c)
				}
			} else {
				a.addTopic(d)
	...
	undoExecute: function(b) {
		var a = [];
		_.each(this._models, function(c) {
			a.push(c.clone())
		});
		_.each(this._models, function(d) {
			var e = d.getId();
...
mindplot.commands.ChangeFeatureToTopicCommand = new Class({
	...
	execute: function(d) {
		var b = d.findTopics(this._topicId)[0];
...
mindplot.commands.RemoveFeatureFromTopicCommand = new Class({
	Extends: mindplot.Command,
	initialize: function(a, b) {
		$assert($defined(a), "topicId can not be null");
	...
	execute: function(c) {
		var a = c.findTopics(this._topicId)[0];
		var b = a.findFeatureById(this._featureId);
		a.removeFeature(b);
		this._oldFeature = b
	},
	undoExecute: function(b) {
		var a = b.findTopics(this._topicId)[0];
...
mindplot.commands.AddFeatureToTopicCommand = new Class({
	...
	execute: function(c) {
		var b = c.findTopics(this._topicId)[0];
...
mindplot.commands.AddRelationshipCommand = new Class({
	Extends: mindplot.Command,
	initialize: function(a) {
		$assert(a, "Relationship model can not be null");
		this.parent();
		this._model = a
	},
	execute: function(a) {
		var b = a.addRelationship(this._model);
		b.setOnFocus(true)
	},
	undoExecute: function(b) {
		var a = b.findRelationships(this._model.getId());
...
mindplot.commands.MoveControlPointCommand = new Class({
	...
	execute: function(b) {
		var a = this._line.getModel();
		switch (this._point) {
			case 0:
				a.setSrcCtrlPoint(this._controlPoint.clone());
				...
				break;
			case 1:
				a.setDestCtrlPoint(this._controlPoint.clone());
				this._wasCustom = this._line.getLine().isDestControlPointCustom();
	...
	undoExecute: function(c) {
		...
		switch (this._point) {
			case 0:
				if ($defined(this._oldControlPoint)) {
					a.setFrom(this._originalEndPoint.x, this._originalEndPoint.y);
			...
			case 1:
				if ($defined(this._oldControlPoint)) {
					a.setTo(this._originalEndPoint.x, this._originalEndPoint.y);
		...
		this._line.getLine().updateLine(this._point);
		if (this._line.isOnFocus()) {
			this._ctrlPointControler.setLine(a);
			a._refreshShape()
		}
	}
});
mindplot.widget.ModalDialogNotifier = new Class({
	initialize: function() {},
	show: function(c, e) {
		$assert(c, "message can not be null");
...
$notify = function(a) {
	toolbarNotifier.logMessage(a)
};
mindplot.widget.ToolbarItem = new Class({
	Implements: mindplot.Events,
	initialize: function(c, b, a) {
		$assert(c, "buttonId can not be null");
...
mindplot.widget.ToolbarPaneItem = new Class({
	...
	_init: function() {
		var a = this.buildPanel();
		a.css("cursor", "default");
		var b = this.getButtonElem();
		var c = this;
		this._tip = new mindplot.widget.FloatingTip(b, {
			html: true,
			placement: "bottom",
			content: function() {
				return c._updateSelectedItem()
			},
...
mindplot.widget.NoteEditor = new Class({
	...
	initialize: function(b) {
		...
		this.parent($msg("Note"), {
			...
			closeButton: true,
			acceptButton: true,
			removeButton: typeof b.getValue() != "undefined",
			onEventData: {
				model: this._model
			}
		});
	...
	_buildPanel: function(b) {
		var a = $("<div></div>").css("padding-top", "5px");
		var c = $("<form></form>").attr({
			action: "none",
			id: "noteFormId"
		});
		var d = $("<textarea></textarea autofocus>").attr({
	...
	onAcceptClick: function(a) {
		a.data.dialog._submitForm(a.data.model)
	},
	_submitForm: function(b) {
		var a = this._native.find("textarea");
		if (a.val()) {
			b.setValue(a.val())
		}
		this.close()
...
mindplot.widget.LinkEditor = new Class({
	...
	_buildPanel: function(d) {
		...
		var c = $('<input id="inputUrl"/>').attr({
			placeholder: "http://www.example.com/",
			required: "true",
			autofocus: "autofocus",
			"class": "form-control"
		});
		c.on("keypress", function(i) {
			i.stopPropagation()
		});
		if (d.getValue() != null) {
			c.val(d.getValue())
		...
		b.click(function() {
			window.open(c.val(), "_blank", "status=1,width=700,height=450,resize=1")
		});
		...
		this.form.append(f);
		var e = this;
		this.form.unbind("submit").submit(function(j) {
			j.preventDefault();
			if (e.checkURL(c.val())) {
				e.cleanError();
				var i = c.val();
	...
	checkURL: function(a) {
		var b = /^(http|https|ftp):\/\/[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/i;
		return (b.test(a))
	},
	onAcceptClick: function(a) {
...
mindplot.widget.LinkIconTooltip = new Class({
	...
	initialize: function(a) {
		$assert(a, "linkIcon can not be null");
		var b = $(a.getImage()._peer._native);
		this.parent(b, {
			content: this._buildContent(a),
			html: true,
			placement: "bottom",
			container: "body",
			title: $msg("LINK"),
	...
	_buildContent: function(b) {
		var a = $("<div></div>").css({
			padding: "5px",
			width: "100%"
		});
		var f = $("<div></div>").text("URL: " + b.getModel().getUrl()).css({
			"white-space": "pre-wrap",
			"word-wrap": "break-word"
		});
		a.append(f);
		...
		var d = $("<img>").prop("src", "http://free.pagepeeker.com/v2/thumbs.php?size=m&url=" + b.getModel().getUrl()).prop("img", b.getModel().getUrl()).prop("alt", b.getModel().getUrl());
		d.css("padding", "5px");
		var e = $("<a></a>").attr({
			href: b.getModel().getUrl(),
...
mindplot.widget.KeyboardShortcutTooltip = new Class({
	...
	initialize: function(a, d) {
		$assert(a, "buttonElem can not be null");
...
mindplot.widget.ColorPalettePanel = new Class({
	...
	_load: function() {
		if (!mindplot.widget.ColorPalettePanel._panelContent) {
			...
			$.ajax({
				...
				error: function() {
					a = "<div>Sorry, your request failed :(</div>"
				}
			});
	...
	buildPanel: function() {
		var c = $('<div class="toolbarPanel"></div>').attr("id", this._buttonId + "colorPalette");
		c.html(this._load());
...
mindplot.widget.ListToolbarPanel = new Class({
	...
	_initPanel: function() {
		var a = this;
		this.getPanelElem().children("div").bind("click", function(b) {
			b.stopPropagation();
			a.hide();
	...
	_updateSelectedItem: function() {
		var a = this.getPanelElem();
		var b = a.find("div");
		var c = this.getModel().getValue();
		_.each(b, function(e) {
			var d = $defined($(e).attr("model")) ? $(e).attr("model") : $(e).attr("id");
			$assert(d, "elemValue can not be null");
			if (d == c) {
				$(e).attr("class", "toolbarPanelLinkSelectedLink")
			} else {
				$(e).attr("class", "toolbarPanelLink")
			}
		});
		return a
	}
...
mindplot.widget.FontFamilyPanel = new Class({
	...
	buildPanel: function() {
		var a = $("<div class='toolbarPanel' id='fontFamilyPanel'></div>");
...
mindplot.widget.FontSizePanel = new Class({
	...
	buildPanel: function() {
		var a = $("<div class='toolbarPanel' id='fontSizePanel'></div>");
...
mindplot.widget.TopicShapePanel = new Class({
	...
	buildPanel: function() {
		var a = $("<div class='toolbarPanel' id='topicShapePanel'></div>");
...
mindplot.widget.IconPanel = new Class({
	...
	buildPanel: function() {
		var g = $('<div class="toolbarPanel" id="IconsPanel"></div>').css({
			width: 245,
			height: 230
		});
		...
		var f = 0;
		for (var c = 0; c < mindplot.ImageIcon.prototype.ICON_FAMILIES.length; c = c + 1) {
			var l = mindplot.ImageIcon.prototype.ICON_FAMILIES[c].icons;
			for (var b = 0; b < l.length; b = b + 1) {
				var h;
				if ((f % 12) == 0) {
					h = $("<div></div>");
					g.append(h)
...
mindplot.widget.IMenu = new Class({
	initialize: function(c, a, b) {
		...
		this._designer.addEvent("modelUpdate", function() {
			d.setRequireChange(true)
		})
	},
	clear: function() {
		_.each(this._toolbarElems, function(a) {
			a.hide()
		})
	},
	...
	setRequireChange: function(a) {
		this._mindmapUpdated = a
	}
});
mindplot.widget.Menu = new Class({
	Extends: mindplot.widget.IMenu,
	initialize: function(H, v, j, A, d) {
		this.parent(H, v, j);
		d = !$defined(d) ? "" : d;
		var c = d + "css/widget";
		$("#" + this._containerId).bind("click", function(J) {
			J.stopPropagation();
			return false
		});
		$("#" + this._containerId).bind("dblclick", function(J) {
			J.stopPropagation();
			return false
		});
		var o = H.getModel();
		...
		this._addButton("export", false, false, function() {
			BootstrapDialog.Request.active = new BootstrapDialog.Request("c/maps/" + j + "/exportf", $msg("EXPORT"), {
				cancelButton: true,
				closeButton: true
			})
		});
		...
		var b = this._addButton("undoEdition", false, false, function() {
			H.undo()
		});
		if (b) {
			b.disable()
		...
		var G = this._addButton("redoEdition", false, false, function() {
			H.redo()
		});
		if (G) {
			G.disable()
		...
		if (G && b) {
			H.addEvent("modelUpdate", function(J) {
				if (J.undoSteps > 0) {
					b.enable()
				} else {
					b.disable()
		...
		this._addButton("topicRelation", true, false, function(J) {
			H.showRelPivot(J)
		});
		...
		if (E) {
			this._addButton("shareIt", false, false, function() {
				BootstrapDialog.Request.active = new BootstrapDialog.Request("c/maps/" + j + "/sharef", $msg("COLLABORATE"), {
					closeButton: true,
					cancelButton: true
				});
		...
		if (a) {
			this._addButton("publishIt", false, false, function() {
				BootstrapDialog.Request.active = new BootstrapDialog.Request("c/maps/" + j + "/publishf", $msg("PUBLISH"), {
					closeButton: true,
					cancelButton: true
				});
				H.onObjectFocusEvent()
		...
		if (y) {
			this._addButton("history", false, false, function() {
				BootstrapDialog.Request.active = new BootstrapDialog.Request("c/maps/" + j + "/historyf", $msg("HISTORY"), {
					closeButton: true,
					cancelButton: true
				});
				H.onObjectFocusEvent()
		...
		if (x) {
			...
			x.bind("click", function(J) {
				window.open("https://www.youtube.com/tv?vq=medium#/watch?v=rKxZwNKs9cE", "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=yes, copyhistory=no, width=" + w + ", height=" + u + ", top=" + s + ", left=" + g);
				J.preventDefault()
			})
		}
	},
	_registerEvents: function(a) {
		var b = this;
		_.each(this._toolbarElems, function(c) {
			c.addEvent("show", function() {
				b.clear()
			})
		});
		a.addEvent("onblur", function() {
			var d = a.getModel().filterSelectedTopics();
			var c = a.getModel().filterSelectedRelationships();
			_.each(b._toolbarElems, function(g) {
				var f = g.isTopicAction();
				var e = g.isRelAction();
				if (f || e) {
					if ((f && d.length != 0) || (e && c.length != 0)) {
						g.enable()
					} else {
						g.disable()
					}
	...
	_addButton: function(f, c, b, e) {
		var g = this;
		var a = null;
		if ($("#" + f)) {
			var d = new mindplot.widget.ToolbarItem(f, function(h) {
				e(h);
				g.clear()
			}, {
				topicAction: c,
	...
	_registerTooltip: function(b, d, a) {
		if ($("#" + b)) {
			var c = d;
			if (a) {
				a = Browser.Platform.mac ? a.replace("meta+", "⌘") : a.replace("meta+", "ctrl+");
				c = c + " (" + a + ")"
			}
...
mindplot.layout.EventBusDispatcher = new Class({
	...
	_nodeResizeEvent: function(a) {
		this._layoutManager.updateNodeSize(a.node.getId(), a.size)
	},
	_nodeMoveEvent: function(a) {
		this._layoutManager.moveNode(a.node.getId(), a.position)
	},
	_nodeDisconnectEvent: function(a) {
		this._layoutManager.disconnectNode(a.getId())
	},
	_nodeConnectEvent: function(a) {
		this._layoutManager.connectNode(a.parentNode.getId(), a.childNode.getId(), a.childNode.getOrder())
	},
	_nodeShrinkEvent: function(a) {
		this._layoutManager.updateShrinkState(a.getId(), a.areChildrenShrunken())
	},
	_nodeAdded: function(a) {
		if (a.getId() != 0) {
			this._layoutManager.addNode(a.getId(), {
				width: 10,
				height: 10
			}, a.getPosition());
			this._layoutManager.updateShrinkState(a.getId(), a.areChildrenShrunken())
	...
	_nodeRemoved: function(a) {
		this._layoutManager.removeNode(a.getId())
	},
	_doLayout: function() {
...
mindplot.layout.ChangeEvent = new Class({
	initialize: function(a) {
		$assert(!isNaN(a), "id can not be null");
		this._id = a;
		this._position = null;
		this._order = null
	},
	...
	setOrder: function(a) {
		$assert(!isNaN(a), "value can not be null");
		this._order = a
	},
	setPosition: function(a) {
		$assert(a, "value can not be null");
		this._position = a
	},
	toString: function() {
...
mindplot.layout.LayoutManager = new Class({
	...
	initialize: function(d, b) {
		$assert($defined(d), "rootNodeId can not be null");
	...
	find: function(a) {
		return this._treeSet.find(a)
	},
	moveNode: function(c, a) {
		$assert($defined(c), "id cannot be null");
	...
	connectNode: function(c, b, a) {
		$assert($defined(c), "parentId cannot be null");
		$assert($defined(b), "childId cannot be null");
		$assert($defined(a), "order cannot be null");
		this._layout.connectNode(c, b, a);
		return this
	},
	disconnectNode: function(a) {
		$assert($defined(a), "id can not be null");
		this._layout.disconnectNode(a);
		return this
	},
	addNode: function(d, c, b) {
		$assert($defined(d), "id can not be null");
	...
	removeNode: function(b) {
		...
		if (this._treeSet.getParent(a)) {
			this.disconnectNode(b)
		}
		this._treeSet.remove(b);
		return this
	},
	predict: function(h, f, b, e) {
		$assert($defined(h), "parentId can not be null");
	...
	plot: function(a, c) {
		$assert(a, "containerId cannot be null");
		c = c || {
			width: 200,
			height: 200
		};
	...
	_flushEvents: function() {
		_.each(this._events, function(a) {
			this.fireEvent("change", a)
		}, this);
		this._events = []
	},
	_collectChanges: function(a) {
		if (!a) {
			a = this._treeSet.getTreeRoots()
		}
		_.each(a, function(c) {
			if (c.hasOrderChanged() || c.hasPositionChanged()) {
				var d = c.getId();
				var b = this._events.some(function(e) {
					return e.id == d
				});
				if (!b) {
					b = new mindplot.layout.ChangeEvent(d)
				}
...
mindplot.layout.Node = new Class({
	initialize: function(d, b, a, c) {
		$assert(typeof d === "number" && isFinite(d), "id can not be null");
		$assert(b, "size can not be null");
	...
	setShrunken: function(a) {
		this._setProperty("shrink", a)
	},
	areChildrenShrunken: function() {
		return this._getProperty("shrink")
	},
	setOrder: function(a) {
		$assert(typeof a === "number" && isFinite(a), "Order can not be null. Value:" + a);
		this._setProperty("order", a)
	},
	resetPositionState: function() {
		var a = this._properties.position;
		if (a) {
			a.hasChanged = false
		}
	},
	resetOrderState: function() {
		var a = this._properties.order;
		if (a) {
			a.hasChanged = false
		}
	},
	resetFreeState: function() {
		var a = this._properties.freeDisplacement;
		if (a) {
			a.hasChanged = false
		}
	},
	...
	setSize: function(a) {
		$assert($defined(a), "Size can not be null");
		this._setProperty("size", Object.clone(a))
	},
	getSize: function() {
		return this._getProperty("size")
	},
	setFreeDisplacement: function(a) {
		$assert($defined(a), "Position can not be null");
		$assert($defined(a.x), "x can not be null");
		$assert($defined(a.y), "y can not be null");
	...
	getFreeDisplacement: function() {
		var a = this._getProperty("freeDisplacement");
		return (a || {
			x: 0,
			y: 0
		})
	...
	setPosition: function(a) {
		...
		if (b == null || Math.abs(b.x - a.x) > 2 || Math.abs(b.y - a.y) > 2) {
			this._setProperty("position", a)
		}
	},
	_setProperty: function(a, b) {
		...
		if (JSON.stringify(c.value) != JSON.stringify(b)) {
			c.oldValue = c.value;
			c.value = b;
			c.hasChanged = true
		}
...
mindplot.layout.RootedTreeSet = new Class({
	...
	getSiblings: function(a) {
		$assert(a, "node cannot be null");
		if (!$defined(a._parent)) {
			return []
		}
		var b = a._parent._children.filter(function(c) {
			return c != a
		});
		return b
	},
	hasSinglePathToSingleLeaf: function(a) {
		$assert(a, "node cannot be null");
		return this._hasSinglePathToSingleLeaf(a)
	},
	_hasSinglePathToSingleLeaf: function(b) {
		var a = this.getChildren(b);
		if (a.length == 1) {
			return this._hasSinglePathToSingleLeaf(a[0])
		}
		return a.length == 0
	},
	isStartOfSubBranch: function(a) {
		return this.getSiblings(a).length > 0 && this.getChildren(a).length == 1
	},
	isLeaf: function(a) {
		$assert(a, "node cannot be null");
	...
	_plot: function(d, e, m) {
		...
		l.attr("fill", a);
		var k = {
			x: l.attr("x") - d.width / 2 + l.attr("width") / 2,
			y: l.attr("y") - d.height / 2 + l.attr("height") / 2
		};
		var n = {
			width: l.attr("width"),
		...
		l.click(function() {
			console.log("[id:" + e.getId() + ", order:" + e.getOrder() + ", position:(" + k.x + "," + k.y + "), size:" + n.width + "x" + n.height + ", freeDisplacement:(" + e.getFreeDisplacement().x + "," + e.getFreeDisplacement().y + ")]")
		});
		o.click(function() {
			console.log("[id:" + e.getId() + ", order:" + e.getOrder() + ", position:(" + k.x + "," + k.y + "), size:" + n.width + "x" + n.height + ", freeDisplacement:(" + e.getFreeDisplacement().x + "," + e.getFreeDisplacement().y + ")]")
		});
	...
	updateBranchPosition: function(f, a) {
		var b = f.getPosition();
		f.setPosition(a);
		var d = b.x - a.x;
		var g = b.y - a.y;
		...
		var e = this;
		_.each(c, function(h) {
			e.shiftBranchPosition(h, d, g)
		})
	},
	shiftBranchPosition: function(e, c, f) {
		var a = e.getPosition();
		e.setPosition({
			x: a.x + c,
			y: a.y + f
		...
		var d = this;
		_.each(b, function(g) {
			d.shiftBranchPosition(g, c, f)
		})
	},
	getSiblingsInVerticalDirection: function(b, d) {
		var a = this.getParent(b);
		var c = this.getSiblings(b).filter(function(g) {
			var f = b.getPosition().x > a.getPosition().x ? g.getPosition().x > a.getPosition().x : g.getPosition().x < a.getPosition().x;
	...
	getBranchesInVerticalDirection: function(d, f) {
		var b = this.getRootNode(d);
		var a = this.getChildren(b).filter(function(g) {
			return this._find(d.getId(), g)
		}, this);
		var c = a[0];
		var e = this.getSiblings(c).filter(function(h) {
			var g = d.getPosition().x > b.getPosition().x ? h.getPosition().x > b.getPosition().x : h.getPosition().x < b.getPosition().x;
...
mindplot.layout.ChildrenSorterStrategy = new Class({
	initialize: function() {},
	computeChildrenIdByHeights: function(b, a) {
		throw "Method must be implemented"
	},
	computeOffsets: function(b, a) {
		throw "Method must be implemented"
	},
	insert: function(c, b, d, a) {
		throw "Method must be implemented"
	},
	detach: function(b, a) {
		throw "Method must be implemented"
	},
	predict: function(d, b, c, a, e) {
		throw "Method must be implemented"
	},
	verify: function(b, a) {
		throw "Method must be implemented"
	},
	getChildDirection: function(b, a) {
		throw "Method must be implemented"
	},
	toString: function() {
...
mindplot.layout.AbstractBasicSorter = new Class({
	...
	computeChildrenIdByHeights: function(c, b) {
		var a = {};
	...
	_computeChildrenHeight: function(g, f, c) {
		var b = f.getSize().height + (this._getVerticalPadding() * 2);
		var a;
		var e = g.getChildren(f);
		if (e.length == 0 || f.areChildrenShrunken()) {
			a = b
		} else {
			var d = 0;
			_.each(e, function(h) {
				d += this._computeChildrenHeight(g, h, c)
			}, this);
			a = Math.max(b, d)
		} if (c) {
			c[f.getId()] = a
	...
	_getSortedChildren: function(c, b) {
		var a = c.getChildren(b);
		a.sort(function(e, d) {
			return e.getOrder() - d.getOrder()
		});
		return a
	},
	_getRelativeDirection: function(b, a) {
		var c = a.x - b.x;
		return c >= 0 ? 1 : -1
	}
});
...
mindplot.layout.BalancedSorter = new Class({
	...
	initialize: function() {},
	predict: function(a, h, n, t, m) {
		if (m) {
			$assert($defined(t), "position cannot be null for predict in free positioning");
		...
		var q = a.getRootNode(h);
		if (n) {
			$assert($defined(t), "position cannot be null for predict in dragging");
			...
			if (f.length == 0 && b == i && s) {
				return [n.getOrder(), n.getPosition()]
			}
		}
		if (!t) {
			var p = this._getChildrenForOrder(h, a, 0);
			var c = this._getChildrenForOrder(h, a, 1)
		}
		var o = t ? (t.x > q.getPosition().x ? 0 : 1) : ((p.length - c.length) > 0 ? 1 : 0);
		var r = o % 2 == 0 ? 1 : -1;
		var d = this._getChildrenForOrder(h, a, o).filter(function(u) {
			return u != n
		});
		if (d.length == 0) {
			return [o, {
				x: h.getPosition().x + r * (h.getSize().width / 2 + mindplot.layout.BalancedSorter.INTERNODE_HORIZONTAL_PADDING * 2),
				y: h.getPosition().y
			}]
		}
		var j = null;
		...
		_.each(d, function(w, u) {
			var v = w.getPosition();
			if (t.y > v.y) {
				yOffset = w == g ? w.getSize().height + mindplot.layout.BalancedSorter.INTERNODE_VERTICAL_PADDING * 2 : (d[u + 1].getPosition().y - w.getPosition().y) / 2;
				j = [w.getOrder() + 2, {
					x: v.x,
					y: v.y + yOffset
				}]
		...
		if (!j) {
			...
			j = [t.x > 0 ? 0 : 1, {
				x: e.getPosition().x,
				y: e.getPosition().y - e.getSize().height - mindplot.layout.BalancedSorter.INTERNODE_VERTICAL_PADDING * 2
			}]
		}
		return j
	},
	insert: function(g, j, a, d) {
		var b = this._getChildrenForOrder(j, g, d);
		if (b.length == 0) {
			a.setOrder(d % 2);
			return
		}
	...
	detach: function(d, c) {
		...
		_.each(a, function(f, e) {
			if (f.getOrder() > c.getOrder()) {
				f.setOrder(f.getOrder() - 2)
			}
		});
		c.setOrder(c.getOrder() % 2 == 0 ? 0 : 1)
	},
	computeOffsets: function(j, c) {
		$assert(j, "treeSet can no be null.");
		$assert(c, "node can no be null.");
		var b = this._getSortedChildren(j, c);
		var k = b.map(function(i) {
			return {
				id: i.getId(),
				order: i.getOrder(),
				width: i.getSize().width,
				height: this._computeChildrenHeight(j, i)
			}
		}, this).reverse();
		var e = 0;
		var a = 0;
	...
	verify: function(f, e) {
		var d = this._getChildrenForOrder(e, f, e.getOrder());
	...
	getChildDirection: function(a, b) {
		return b.getOrder() % 2 == 0 ? 1 : -1
	},
	toString: function() {
		return "Balanced Sorter"
	},
	_getChildrenForOrder: function(b, c, a) {
		return this._getSortedChildren(c, b).filter(function(d) {
			return d.getOrder() % 2 == a % 2
		})
	},
	_getVerticalPadding: function() {
...
mindplot.layout.SymmetricSorter = new Class({
	Extends: mindplot.layout.AbstractBasicSorter,
	initialize: function() {},
	predict: function(a, g, p, w, m) {
		var n = this;
		var t = a.getRootNode(g);
		if (m) {
			$assert($defined(w), "position cannot be null for predict in free positioning");
		...
		if (!p) {
			var s = n._getRelativeDirection(t.getPosition(), g.getPosition());
			var w = {
				x: g.getPosition().x + s * (g.getSize().width + mindplot.layout.SymmetricSorter.INTERNODE_HORIZONTAL_PADDING),
				y: g.getPosition().y
			};
		...
		if (e.length == 0 && c == h && v) {
			return [p.getOrder(), p.getPosition()]
		}
		var x = a.getChildren(g);
		if (x.length == 0) {
			var w = {
				x: g.getPosition().x + h * (g.getSize().width + mindplot.layout.SymmetricSorter.INTERNODE_HORIZONTAL_PADDING),
				y: g.getPosition().y
			};
			return [0, w]
		} else {
			...
			for (var r = 0; r < x.length; r++) {
				...
				if (!o && w.y > b.getPosition().y) {
					var q = (a.getParent(p) && a.getParent(p).getId() == g.getId()) ? f.getOrder() : f.getOrder() + 1;
					var w = {
						x: b.getPosition().x,
						y: b.getPosition().y + b.getSize().height + mindplot.layout.SymmetricSorter.INTERNODE_VERTICAL_PADDING * 2
					};
					return [q, w]
				}
				if (o && w.y > b.getPosition().y && w.y < o.getPosition().y) {
					if (o.getId() == p.getId() || b.getId() == p.getId()) {
						return [p.getOrder(), p.getPosition()]
					} else {
						var q = w.y > p.getPosition().y ? o.getOrder() - 1 : b.getOrder() + 1;
						var w = {
							x: b.getPosition().x,
							y: b.getPosition().y + (o.getPosition().y - b.getPosition().y) / 2
						};
						return [q, w]
					}
				}
		...
		var w = {
			x: d.getPosition().x,
			y: d.getPosition().y - d.getSize().height - mindplot.layout.SymmetricSorter.INTERNODE_VERTICAL_PADDING * 2
		};
		return [0, w]
	},
	insert: function(f, d, g, a) {
	...
	detach: function(f, e) {
		...
		for (var b = e.getOrder() + 1; b < c.length; b++) {
			var g = c[b];
			g.setOrder(g.getOrder() - 1)
		}
		e.setOrder(0)
	},
	computeOffsets: function(g, c) {
		$assert(g, "treeSet can no be null.");
		$assert(c, "node can no be null.");
		...
		var j = b.map(function(i) {
			return {
				...
				width: i.getSize().width,
				height: this._computeChildrenHeight(g, i)
			}
		}, this).reverse();
		var h = 0;
		_.each(j, function(i) {
	...
	verify: function(d, c) {
		var b = this._getSortedChildren(d, c);
		for (var a = 0; a < b.length; a++) {
			$assert(b[a].getOrder() == a, "missing order elements")
		}
	},
	getChildDirection: function(d, g) {
		$assert(d, "treeSet can no be null.");
		$assert(d.getParent(g), "This should not happen");
		var a;
		var b = d.getRootNode(g);
		if (d.getParent(g) == b) {
			a = Math.sign(b.getPosition().x)
		} else {
			var c = d.getParent(g);
			var e = d.getParent(c);
			var f = e.getSorter();
...
mindplot.layout.GridSorter = new Class({
	Extends: mindplot.layout.AbstractBasicSorter,
	computeOffsets: function(k, c) {
		$assert(k, "treeSet can no be null.");
		$assert(c, "node can no be null.");
		$assert("order can no be null.");
		var b = this._getSortedChildren(k, c);
		var n = this;
		var m = b.map(function(i) {
			return {
				id: i.getId(),
				height: n._computeChildrenHeight(k, i)
			}
		});
		var p = {};
...
mindplot.layout.OriginalLayout = new Class({
	...
	createNode: function(e, b, a, c) {
		$assert($defined(e), "id can not be null");
		$assert(b, "size can not be null");
		$assert(a, "position can not be null");
		$assert(c, "type can not be null");
	...
	connectNode: function(f, c, a) {
		var b = this._treeSet.find(f);
	...
	layout: function() {
		var a = this._treeSet.getTreeRoots();
		_.each(a, function(c) {
			var d = c.getSorter();
	...
	_layoutChildren: function(d, g) {
		var c = d.getId();
		var b = this._treeSet.getChildren(d);
		var k = this._treeSet.getParent(d);
		var i = b.some(function(o) {
			return o.hasOrderChanged()
		});
		var l = b.some(function(o) {
			return o.hasSizeChanged()
		});
		var e = g[c];
		...
		if (i || l || f || a) {
			...
			_.each(b, function(u) {
				...
				if ((s > 0 && o.x < 0) || (s < 0 && o.x > 0)) {
					u.resetFreeDisplacement();
					u.setFreeDisplacement({
						x: -o.x,
						y: o.y
					})
				}
				...
				var p = {
					x: r + t.x,
					y: q + t.y + h._calculateAlignOffset(d, u, g)
				};
		...
		_.each(b, function(o) {
			this._layoutChildren(o, g)
		}, this)
	},
	_calculateAlignOffset: function(c, f, b) {
		if (f.isFree()) {
			return 0
		}
		var e = 0;
		...
		if (this._treeSet.isStartOfSubBranch(f) && this._branchIsTaller(f, b)) {
			if (this._treeSet.hasSinglePathToSingleLeaf(f)) {
				e = b[f.getId()] / 2 - (a + f.getSorter()._getVerticalPadding() * 2) / 2
			} else {
				e = this._treeSet.isLeaf(f) ? 0 : -(a - d) / 2
			}
		} else {
			if (d > a) {
				if (this._treeSet.getSiblings(f).length > 0) {
					e = 0
				} else {
					e = d / 2 - a / 2
				}
	...
	_branchIsTaller: function(b, a) {
		return a[b.getId()] > (b.getSize().height + b.getSorter()._getVerticalPadding() * 2)
	},
	_fixOverlapping: function(c, b) {
		var a = this._treeSet.getChildren(c);
		if (c.isFree()) {
			this._shiftBranches(c, b)
		}
		_.each(a, function(d) {
			this._fixOverlapping(d, b)
		}, this)
	},
	_shiftBranches: function(f, d) {
		var a = [f];
		var b = this._treeSet.getSiblingsInVerticalDirection(f, f.getFreeDisplacement().y);
		var e = f;
		_.each(b, function(h) {
			var g = a.some(function(j) {
				return this._branchesOverlap(j, h, d)
			}, this);
			if (!h.isFree() || g) {
				var i = f.getFreeDisplacement().y;
				this._treeSet.shiftBranchPosition(h, 0, i);
				a.push(h)
			}
		}, this);
		var c = this._treeSet.getBranchesInVerticalDirection(f, f.getFreeDisplacement().y).filter(function(g) {
			return !a.contains(g)
		});
		_.each(c, function(h) {
			var g = f.getFreeDisplacement().y;
			this._treeSet.shiftBranchPosition(h, 0, g);
			a.push(h);
			e = h
		}, this)
	},
	_branchesOverlap: function(g, f, d) {
		if (g == f) {
			return false
		}
